...
//
//  Licencing info 
//  FileHash: C8B04FB7-7AF1-48C2-85B9-76A80477285E
...
//
//
//  MaxwellSpexTests.swift
//  MaxwellSpexTests
//
//  Created by mark maxwell on 5/9/20.
//  Copyright © 2020. All rights reserved.
//

@testable import M_A_X_W_E_L_L
import XCTest

class MaxwellSpexTests: XCTestCase {
    override func setUp() {
    ...
    func testLicensing() {
        let expectation1 = expectation(description: "should license contract")
        let model = LicensingModel()
        model.state.email = "ruty@fruty.com"
        model.state.name = "Ruty Fruty"
        model.prepareLicense(licensedProdutId: nil, mocking: true)
        model.uploadLicensedProduct { license in

            guard let license = license else {
                XCTAssertNotNil(nil)
                return
            }
            XCTAssertTrue(license.isReadyToDownload)
    ...
    func testStorage() {
        let expectation1 = expectation(description: "should store and read back")
        let tableName = "test\(UUID().uuidString)"
        let table = Storage.Table(name: tableName)
        let db = Storage.Database<LicensingModel.License?>(table: table) { nil }

        let license = LicensingModel.License()

        db.store(item: license)
        guard let restored = db.restore() else {
            XCTAssert(false)
            return
        }

        XCTAssert(license.id == restored.id)

        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            let db2 = Storage.Database<LicensingModel.License?>(table: table) { nil }

            guard let restored2 = db2.restore() else {
                XCTAssert(false)
                return
            }

            XCTAssert(license.id == restored2.id)
            expectation1.fulfill()
        }
    ...
    func testConfig() {
        FireConfig.launchSetup()
        FireConfig.envKeys.enumerated().forEach {
            print("idx:\($0) \(FireConfig.env[keyPath: $1])")
            if FireConfig.env[keyPath: $1].count == 0 {
                print("error:\($0) \(FireConfig.env[keyPath: $1])")
            }
            XCTAssertGreaterThan(FireConfig.env[keyPath: $1].count, 0)
        }
    }
}
