...
//
//  Licencing info 
//  FileHash: BE73B70A-EAE0-408D-BF6A-5B12CAAB0D3F
...
typealias ApplicationLaunchOptions = [UIApplication.LaunchOptionsKey: Any]?
@UIApplicationMain
class AppDelegate: UIResponder, UIApplicationDelegate {
    func application(_: UIApplication, didFinishLaunchingWithOptions options: ApplicationLaunchOptions) -> Bool {
        // Override point for customization after application launch.
        UNUserNotificationCenter.current().delegate = Permissions.shared

        FirebaseCoordinator.launchSetup()
        FireConfig.launchSetup()
        FireStorage.launchSetup()
        ATracker.launchSetup(options: options)
        AppStore.shared.launchSetup()
        Authentication.launchSetup()
        Permissions.launchSetup()
        return true
    }
    ...
    func application(_: UIApplication, didRegisterForRemoteNotificationsWithDeviceToken deviceToken: Data) {
        ...
        Mixpanel.mainInstance().people.addPushDeviceToken(deviceToken as Data)
    }
