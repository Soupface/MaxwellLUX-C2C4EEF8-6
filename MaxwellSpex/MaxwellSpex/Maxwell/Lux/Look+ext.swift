...
//
//  Licencing info 
//  FileHash: 30FA83FF-5DE2-4A43-8C6A-C115C341BDAC
...
extension Look {
    var publishName: String {
        name.alphanumeric.uppercased()
    }

    var publishFileName: String {
        "\(publishName)_v\(meta.publishedVersion)".uppercased()
    }

    var publishPublicShortId: String {
        "\(publishName)-\(meta.creationId.prefix(6))"
    }

    var publishPublicShortIdWithUserScope: String {
        "\(Authentication.userId)/\(publishPublicShortId)"
    }

    var publishPublicFilename: String {
        "\(meta.creationId)/\(publishFileName)"
    }

    var publishPublicId: String {
        "\(Authentication.userId)/\(publishPublicFilename)"
    }

    var publishPublicIdWithExt: String {
        "\(publishPublicId).lux"
    }

    var publishFileNameWithExt: String {
        "\(publishFileName).lux"
    }
}
...
extension Look {
    func copyFeedConfig() {
        UIPasteboard.general.string = "Look.setCurrent(feed:\"\(publishPublicShortIdWithUserScope)\")"
        BannerModel.shared.push(BannerModel.Banner(message: "Copied"))
    }

    func copyInitConfig() {
        UIPasteboard.general.string = "Look.setCurrent(data64:\"\(asJSONString().base64Encoded())\")"
        BannerModel.shared.push(BannerModel.Banner(message: "Copied"))
    }
}
...
extension Look {
    static var tmpFolder = URL(fileURLWithPath: NSTemporaryDirectory(),
                               isDirectory: true)

    var QRLuxTempFileURL: URL {
        let temporaryFilename = "\(publishFileName)-lux.png"

        return Self.tmpFolder.appendingPathComponent(temporaryFilename)
    }

    var tempFileURL: URL {
        let temporaryFilename = "\(publishFileName).lux"
...
extension Look {
    func publishedVersion(assetURL: URL, webURL: URL, qrURL: URL, shortURL: URL) -> Look {
        let instance = clone()
        instance.meta.assetURL = assetURL.absoluteString
        instance.meta.webURL = webURL.absoluteString
        instance.meta.qrURL = qrURL.absoluteString
        instance.meta.shortURL = shortURL.absoluteString
        instance.meta.publishedAt = Date()
        return instance
    }
}
