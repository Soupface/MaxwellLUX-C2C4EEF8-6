...
//
//  Licencing info 
//  FileHash: A72DBBE1-F48A-44CC-9C1E-0919BF90DE39
...
//  MaxwellSpex
//
//  Created by mark maxwell on 5/12/20.
...
class AppStore: ObservableObject {
    struct AppStoreContinuation {
        var payment: SKPayment
        var product: SKProduct
    }

    struct Product {
        static var indieId: String {
            let macOS = "com.productId"
            let iOS = "com.productId"
            return UIApplication.isMacCatalyst ? macOS : iOS
        }

        static var startupId: String {
            let macOS = "com.productId"
            let iOS = "com.productId"
            return UIApplication.isMacCatalyst ? macOS : iOS
        }
    ...
    // TODO: verify from app model when presenting the store
    @Published var externalPurchasesFinished: [Purchase] = []
    @Published var products: Set<SKProduct> = []
    @Published var continueAppStoreTransaction: [AppStoreContinuation] = []
    @Published var purchases: RestoreResults?
    @Published var isPurchasing = false
    @Published var isRestoring = false

    init() {
        assert(SwiftyStoreKit.shouldAddStorePaymentHandler == nil, "Singleton expectation")
        SwiftyStoreKit.shouldAddStorePaymentHandler = { [weak self] payment, product in

            self?.continueAppStoreTransaction.append(
                AppStoreContinuation(payment: payment, product: product)
            )

            // move the app in the appstore purchase state
            // later we need to resolve with  resolveContinuedAppStoreTransactionPayments()
            return false
        }
    ...
    func resolveContinuedAppStoreTransactionPayments() {
        continueAppStoreTransaction.forEach { SKPaymentQueue.default().add($0.payment) }
    }
            ...
            for purchase in purchases {
                ...
                case .purchased, .restored:
                    if purchase.needsFinishTransaction {
                        ...
                        self?.externalPurchasesFinished.append(purchase)
    ...
    func retrieveProducts() {
        ATracker.track(.appStore(.startLoadProducts))
        SwiftyStoreKit.retrieveProductsInfo(Product.catalogIds) { [weak self] result in

            guard result.retrievedProducts.first != nil else {
                ATracker.track(.appStore(.finishLoadProducts), props: ["warning": "no products"])
                return
            }
            ATracker.track(.appStore(.finishLoadProducts), props: ["count": result.retrievedProducts.count])

            self?.assign(products: result.retrievedProducts)
        }
    }

    func purchase(product: SKProduct, quantity: Int, completion: @escaping (PurchaseDetails?) -> Void) {
        // make it fail quantity = 0
        guard isPurchasing == false else {
//            assert(false)
            return
        }
        isPurchasing = true

        ATracker.track(.appStore(.startPurhcase))
        SwiftyStoreKit.purchaseProduct(product, quantity: quantity, atomically: true) { [weak self] result in
            ...
            case let .success(purchase):
                ATracker.track(.appStore(.finishPurhcase), props: ["successs": true])
            ...
            case let .error(error):
                ATracker.track(.appStore(.finishPurhcase), props: ["error": error.localizedDescription])
    ...
    func restorePurchasesWithInfoBanners() {
        BannerModel.shared.push(message: "Finishing Purchase...")
        AppStore.shared.restorePurchases { result in
            result.restoredPurchases.count == 0 ?
                BannerModel.shared.push(message: "Please Retry")
                :
                BannerModel.shared.push(message: "Purchase Ready")
        }
    }

    func restorePurchases(completion: ((RestoreResults) -> Void)? = nil) {
        guard isRestoring == false else {
            return
        }
        isRestoring = true
        ATracker.track(.appStore(.startRestore))
        SwiftyStoreKit.restorePurchases(atomically: true) { [weak self] results in

            let info = [
                "restoredPurchases": results.restoredPurchases.count,
                "restoreFailedPurchases": results.restoreFailedPurchases.count,
            ]
            ATracker.track(.appStore(.finishRestore), props: info)
...
extension AppStore {
    func assign(products: Set<SKProduct>) {
...
extension AppStore {
    func launchSetup() {
        AppStore.shared.completePendingTransactions()
    }
}
...
extension AppStore {
    ...
    var hasValidLicense: Bool {
        purchases?.restoredPurchases.count ?? 0 > 0
    }
}
