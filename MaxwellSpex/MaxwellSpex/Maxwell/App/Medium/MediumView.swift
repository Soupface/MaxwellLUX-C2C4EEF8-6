...
//
//  Licencing info 
//  FileHash: 88CF816A-E233-4492-8808-1F3A4602CE9C
...
struct MediumView: View {
    @ObservedObject var model: MediumModel
    @State var showingOptions = false

    var opacity: Double {
        model.ready == true ? 1 : 0
    }

    var progress: CGFloat {
        1.0 - model.webView.estimatedProgress.cgFloat
    }

    var webView: some View {
        WebView(webView: self.model.webView)
            .opacity(self.model.webView.isLoading ? 0.8 : 1.0)
            .animation(.easeInOut)
    }
    ...
    var ready: Bool {
        model.loading || model.ready != true ? false : true
    }
    ...
    func moreOptions() {
        ATracker.track(.home(.tapSettings))
        showingOptions = true
    }

    func tapShare() {
        ATracker.track(.home(.tapShare))
        model.openInShare()
    }
    ...
    var body: some View {
        ZStack {
            GeometryReader { geo in
                Column {
                    LoaderNavBar(webViewStore: self.model)
                        .opacity(0.25)

                    self.bar()

                    self.webView
                        .modifier(Appear3D(finished: self.ready))
                }
                .lux
                .tweak(.normalSurface)
                .if(geo.size.width > geo.size.height) { $0.view.background(self.lux.draw.surfaceComposition.edgesIgnoringSafeArea(.all)).lux.view }
                .view
            }
            ...
            self.model.ready == false ?
                Column {
                    ...
                    Image(systemName: "xmark.icloud")
                        ...
                        .style(.iconLarge)
                        .view
                    Text("Unavailable. Please retry later.")
                        .bold()
                    Spacer()
                }
                .lux
    ...
    var actions: [ActionSheet.Button] {
        [
            .default(Text("Logout"), action: self.logOut),
            .cancel(),
        ]
    }
    ...
    func bar() -> some View {
        NavBar(leftIcons: {
            Group {
                Button(action: { self.model.reload() }) {
                ...
                Button(action: self.moreOptions) {
            ...
            NavigatorNavBar(webViewStore: self.model)
                .lux.view
        },
               rightIcons: {
            Group {
                ...
                .actionSheet(isPresented: self.$showingOptions) {
                    ActionSheet(title: Text("Options"), buttons: self.actions)
                }
                Button(action: self.tapShare) {
