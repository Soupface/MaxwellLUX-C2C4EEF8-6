...
//
//  Licencing info 
//  FileHash: EBD73381-51A0-494B-9766-6F1C572121F8
...
class MediumModel: WebViewStore, WKNavigationDelegate, UINavigationControllerDelegate {
    @Published var ready: Bool? = nil
    @Published var loading = false
    var onConfirm: (([UIColor]) -> Void)?
    var onCancel: (() -> Void)?
    var configured = false
    static let baseURLString = FireConfig.env.pageMediumURL

    init() {
        super.init(rootURLString: MediumModel.baseURLString)
    ...
    override var allowedURLPrefixes: [String] {
        [
            MediumModel.baseURLString,
        ]
    }
}

extension MediumModel {
    // MARK: - WKNavigationDelegate

    func webView(_: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        resolve(navigationAction: navigationAction, decisionHandler: decisionHandler)
    }
    ...
    func webView(_: WKWebView, didFinish _: WKNavigation!) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) { [weak self] in
            ...
            self?.applyOptimisticCustomization()
            self?.verifyIntegrity { [weak self] _ in
                DispatchQueue.main.async {
                    self?.ready = true
                    self?.objectWillChange.send()
                }
                self?.JSrunCustomizationLoop()
            }
...
extension MediumModel {
    func JSrunCustomizationLoop() {
        ...
        let loop = JSmakeScript(interval: 0, script: script)
    ...
    func JSbuildCustomizationScript() -> String {
        "\(JSbuildCustomizationArticleScript())\(JSbuildCustomizationBaseScript())"
    }

    func JSbuildCustomizationArticleScript() -> String {
        return """
            if( document.querySelector('article') != undefined){
                var items = document.all;
                for (var i = 0; i < items.length; i++) {
                   var $this = items[i];
                   if( $this.closest("article") == undefined && $this.tagName != 'ARTICLE'
                       && $this.querySelector("article") == undefined){
                       console.log("remove" + $this.tagName + " " + $this.closest("article") )
                       $this.style.display =  'none'
                    } else{
                       console.log("keep" + $this.tagName + " " + $this.closest("article") )
                    }
                }
            }
        """
    ...
    func JSbuildCustomizationBaseScript() -> String {
        let info: [RemoveInfo] = [
            RemoveInfo(tag: "class", value: "metabar"),
            RemoveInfo(tag: "class", value: "collectionFooter"),
        ]
