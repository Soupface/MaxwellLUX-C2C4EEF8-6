...
//
//  Licencing info 
//  FileHash: D626767D-99EA-417D-B2A1-62E87BF81310
...
struct SpexView: View {
    @ObservedObject var model: AppModel
    @ObservedObject var appstore = AppStore.shared

    func start() {
        ATracker.track(.store(.tapStartPurchase))
        model.state.isLicensing = true
    }

    var spexButton: AnyView {
        Button(action: start) {
            Lux.actionButtonStyle(
                HStack {
                    Text("Continue")
                        .bold()
                        .scaledToFill()
    ...
    var browserKey: String {
        AppStore.shared.hasValidLicense ? FireConfig.env.pageSpexGetStartedURL : FireConfig.env.pageSpexURL
    }

    var browser: WebViewStore {
        AppStore.shared.hasValidLicense ? Navigator.shared.spexGetStarted : Navigator.shared.spex
    }

    var body: some View {
        //        ZStack {
        Group {
            self.model.state.isLicensing ?
                LicensingView(model: self.model, licensing: self.model.licensing)
                .lux.view
                : Column {
                    Group {
                        LoaderNavBar(webViewStore: self.browser)
