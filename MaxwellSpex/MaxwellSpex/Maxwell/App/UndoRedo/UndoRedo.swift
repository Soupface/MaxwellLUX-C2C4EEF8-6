...
//
//  Licencing info 
//  FileHash: E40C4798-ED3E-481C-A0AC-BF236B000995
...
extension UndoRedo {
    var canUndo: Bool {
        undoRedoBusy == false && manager.canUndo
    }

    var canRedo: Bool {
        undoRedoBusy == false && manager.canRedo
    }

    var undoRedoBusy: Bool {
        manager.isUndoing || manager.isRedoing
    }

    func undo() {
        guard undoRedoBusy == false else { return }

        ATracker.track(.editor(.tapUndo))
        manager.undo()
    }
    ...
    func redo() {
        ...
        ATracker.track(.editor(.tapRedo))
        manager.redo()
    }

    func registerUndoLookAction(editor: EditorModel) {
        let snapshot = editor.look.clone()

        manager.registerUndo(withTarget: editor) { [weak self] editor in
            self?.registerUndoLookAction(editor: editor)
            editor.state.session.look = snapshot
            editor.setupObservers()
        }
    }
}
