...
//
//  Licencing info 
//  FileHash: 9DC389D5-6518-402A-AC78-2DFA1AA15790
...
struct AppView: View {
    ...
    @ObservedObject var look = Look.state
    @ObservedObject var lookCurrent = Look.current
    @ObservedObject var download = Download.shared
    @ObservedObject var auth = Authentication.shared

    var body: some View {
        ZStack {
            Authentication.hasSignedUser ?
                AppViewPortrait(model: self.model).lux.view
                : LoginView().lux.view
        }
    }
}

struct AppViewPortrait: View {
    @ObservedObject var model: AppModel
    @ObservedObject var download = Download.shared
    @ObservedObject var look = Look.state
    @ObservedObject var lookCurrent = Look.current
    @ObservedObject var banner = BannerModel.shared

    var showActionSheet: Binding<Bool> {
        Binding<Bool>(
            get: { () -> Bool in
                self.model.isPresentingSheet
            }
        ) {
            $0 == false ? self.model.sheet = nil : nil
        }
    }

    func dismissSheet() {
        if model.publishing.state.publication != nil {
            model.publishing.state.publication = nil
            return
        }
        showActionSheet.wrappedValue = false
    }

    func dismiss() {
        guard model.state.isLicensing == false else {
            model.dismissLicensing()
            return
        }
        model.layout = .normal
    }
    ...
    func restorePurchases() {
        AppStore.shared.restorePurchasesWithInfoBanners()
    }

    func addLookFrom(template: Look) {
        model.sheet = nil
        DispatchQueue.main.async {
            self.model.addLook(template)
        }
    }

    func hidePickTempalte() {
        ATracker.track(.home(.dismissPickTemplate))
        model.sheet = nil
    }

    func openPrintedEdition() {
        UIApplication.shared.open(URL.optional(from: FireConfig.env.mediaDesignCodeLink)!)
    }
    ...
    var body: some View {
        ZStack {
            Column {
                NavBar(leftIcons: {
                    ...
                    Button(action: self.restorePurchases) {
                        HStack {
                            Text("Restore")
                ...
                SpexView(model: self.model)
                    .scaleEffect(self.model.layoutSpexScaleDestValue)
                    .rotation3DEffect(self.model.layoutSpexDestAngle, axis: (x: 1.0, y: 0.0, z: 0.0), anchor: .bottom)
                    .animation(.easeInOut(duration: self.model.layoutAnimationDestDuration.doubleValue))
                    .blur(radius: self.model.layoutDestBlur)
            }
            .lux
            .style(.panel)
            .view
            ...
            Group {
                self.model.mode == .editor ?
                    EditorView(model: self.model.activeEditorModel, layout: .portrait)
                    .lux
                    .view
                    .blur(radius: self.model.layoutSourceBlur)
                    .lux.view
                    :
                    TabsView(model: self.model).lux.view
                    .lux
                    .view
            }
            .opacity(self.model.layoutSpexOpacitySourceValue)
            .animation(.easeInOut(duration: self.model.layoutAnimationSourceDuration.doubleValue))

            BannerTopView()

            Group {
                ImportView(model: self.model.importer)
                RestoreEditorView(model: self.model)
            }

            self.download.isDownloading ?
                Column {
                    Spacer()
                    Row {
                        ActivityIndicator()
        ...
        .sheet(isPresented: showActionSheet) {
            Group {
                self.model.sheet == .templates ?
                    TemplatesContainerView(model: self.model, onDismis: self.dismissSheet, onSelect: self.addLookFrom)
                    .lux.view
                    : ActionSheetContainer(dismiss: self.dismissSheet) {
                        Group {
                            self.model.sheet == .chat ?
                                ZStack {
                                    ActivityIndicator()
                                        .frame(maxWidth: 100)
                                        .blendMode(.difference)
                                    BrowserView(webViewStore: self.model.navigator.chat)
                                        .onAppear {
                                            self.model.navigator.chat.loadIfNeeded()
                                        }
                                }
                                : nil

                            self.model.sheet == .docs ?
                                Column {
                                    BrowserView(webViewStore: self.model.navigator.docs)
                                        .onAppear {
                                            self.model.navigator.docs.loadIfNeeded()
                                        }
                                    Button(action: self.openPrintedEdition) {
                                        Lux.actionButtonStyle(HStack {
                                            Text("Printed Edition").bold()
                            ...
                            self.model.sheet == .streams ?
                                StreamsView(model: self.model, stream: self.model.stream)
                                : nil

                            self.model.sheet == .publish ?
                                PublishingView(model: self.model, publishing: self.model.publishing)
                                : nil
                        }
                    }
                    .lux.view
            }
            .colorScheme(Look.current.scheme.colorScheme)
        }
        .colorScheme(Look.current.scheme.colorScheme)
    }
}
...
struct TabsView: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            //            .background(self.lux.draw.surfaceComposition.edgesIgnoringSafeArea(.all))
        }
    }
    ...
    var content: some View {
        Group {
            model.mode == .store ?
                StoreView(model: model, store: model.store)
                : nil

            model.mode == .lux ?
                LooksView(model: model, stream: model.stream)

                : nil

            model.mode == .home ?
                MediumView(model: model.navigator.medium)
                .lux
                .tweak(.normalSurface)
                .style(.panel)
                .view
        ...
        HStack {
            HStack {
                ...
                Image(systemName: "hexagon")
                    .lux
                    .tweak(.normalSurface)
                    .feature(.disabledOpacity)
                    .tweak(.titleLayout)
                    .style(.iconLarge, .text)
                    .view
                    .overlay(Image(systemName: "triangle.fill").scaleEffect(0.5)
                        .lux
                        .tweak(.normalSurface)
                        .tweak(.titleLayout)
                        .style(.iconLarge, .text)
                        .unless(model.mode == .store) { $0.feature(.disabledOpacity) }
                        .view
                    )
                Spacer()
            }
            ...
            .view
            .onTapGesture { self.model.showStore() }
            ...
            .lux
            .tweak(.normalSurface)
            .unless(model.mode == .lux) { $0.feature(.inactiveOpacity) }
            .feature(.flexibleWidth)
            .feature(.rectangularContentShape)
            .view
            .onTapGesture { self.model.showLux() }
                ...
                Image(systemName: "circle")
                    ...
                    .overlay(
                        Image(systemName: "star.fill").scaleEffect(0.5)
                            ...
                            .tweak(.titleLayout)
                            .style(.iconLarge, .text)
                            .unless(model.mode == .home) { $0.feature(.disabledOpacity) }
                            .view
                    )
                Spacer()
            }
            ...
            .feature(.rectangularContentShape)
            .view
            .onTapGesture { self.model.showHome() }
        }
        .lux
        .tweak(.normalSurface)
        .style(.bar)
