...
//
//  Licencing info 
//  FileHash: 53982889-9886-45D8-911F-8E4020180EB3
...
struct CircleColorButton: View {
    ...
    var binding: Binding<Color> {
        model.colorBinding(keyPath: mode.colorValueKeypath, mode.scheme)
    }
    ...
    var icon: String {
        preferredIcon ?? mode.icon
    }

    var iconColor: Color {
        if case let .background(_, prio: prio) = mode,
            prio == .tertiary {
            return model.look.surface.active
        }
        return color.uiColor().withAlphaComponent(1.0).contrast().uiColor
    }

    var body: some View {
        self.alwaysVisible ||
            self.model.look.scheme == mode.scheme ?
...
struct CircleColorDisplay: View {
    @ObservedObject var model: EditorModel
    var mode: EditorMode.Editor
    var preferredIcon: String?
    var alwaysVisible = false
