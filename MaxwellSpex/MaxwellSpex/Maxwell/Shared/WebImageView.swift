...
//
//  Licencing info 
//  FileHash: 35C1FF9A-248B-4C33-B1AA-30F0E68B46A5
...
extension WebImageView {
    struct Manager {
        ...
        @Atomic var _recent: [RecentImage] = []
        ...
        var recent: [RecentImage] {
            ...
            set {
                _recent = Array(newValue.suffix(13))
            }
        }
    }
    ...
    static func recentImage(for url: URL) -> UIImage? {
        Manager.shared.recent.first(where: { $0.url == url })?.image
    }

    static func storeRecent(image: UIImage?, for url: URL) {
        guard let image = image else { return }
        Manager.shared.recent.append(RecentImage(url: url, image: image))
    }
}

struct WebImageView: View {
    var url: URL?
    var config: ((Image) -> AnyView)?
    @State var image: UIImage?
    @State var loadingURL: URL?

    var ready: Bool {
        loadingURL == nil
    }

    func optimisitcLoad() -> Bool {
        guard let loadingURL = loadingURL,
            let image = Self.recentImage(for: loadingURL) else { return false }
    ...
    func load() {
        ...
        guard let loadingURL = self.loadingURL else { return }
        guard optimisitcLoad() == false else {
            self.loadingURL = nil
            return
        }

        DispatchQueue.global(qos: .background).async {
            let requestURL = loadingURL
            SDWebImageManager.shared.loadImage(with: self.url, progress: { _, _, _ in

            }) { image, _, _, _, _, _ in

                Self.storeRecent(image: image, for: requestURL)
                self.loadingURL = nil

                DispatchQueue.main.async {
                    guard requestURL == self.url else { return }
                    self.image = image
                }
            }
        }
            ...
            image != nil ?
                Group {
                    config == nil ?
                        ...
                        .scaledToFit()
                        .lux.view
                        : config!(Image(uiImage: image!)).lux.view
                }
                : nil
...
struct WebImageView_Previews: PreviewProvider {
    static var previews: some View {
        WebImageView(url: URL.optional(from: "https://res.cloudinary.com/maxwelldesign/image/upload/v1588811374/static/MaxwellLogo1536_az4jet.png")!)
    }
}
