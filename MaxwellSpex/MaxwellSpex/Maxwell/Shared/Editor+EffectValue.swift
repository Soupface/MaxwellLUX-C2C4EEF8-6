...
//
//  Licencing info 
//  FileHash: 0BD3BE77-D138-4704-991C-E90EC901B994
...
struct EffectValueDisplay: View {
    @ObservedObject var model: EditorModel
    var mode: EditorMode.Editor

    var hideTitle = false
    var effectValue: EffectValue {
        model.effectValue(keyPath: mode.effectValueKeypath).wrappedValue
    }
    ...
    var mixWidth: CGFloat {
        max(0, min(1, effectValue.intensity))
    }
    ...
    var body: some View {
        Column {
            ...
            self.hideTitle == false ?
                Group {
                    Row {
                        Text(self.effectValue.name)
                            .lux
                            .trait(self.model.traitNormal)
            ...
            Column {
                Rectangle()
                    .fill(self.lookPalette(color: \.normal.contrast))
                    .frame(height: 8)
                    .opacity((1.0 - self.effectValue.currentRangeScale.doubleValue) * 0.1)
                    .overlay(
                        RoundedRectangle(cornerRadius: self.model.trait.cornerRadius)
                            .fill(self.lookPalette(color: \.active))
                            .frame(height: 4, alignment: .center)
                            .lux
                            .trait(self.model.traitNormal)
                            .tweak(.paddingQuarter, .edgesHorizontal)
                            .feature(.padding)
                            .view
                            .scaleEffect(x: self.mixWidth, y: 1, anchor: .topLeading)
                            .opacity(0.5)
                    )
                    .lux
                    .feature(.hairlinePadding)
                    .view
            }
            Row {
                Button(action: { self.model.setEditingMode(self.mode) }) {
                    Row {
                        Text(self.effectValue.displayValue)
                            .lux
                            .tweak(.titleLayout)
                            .trait(self.model.traitNormal)
                            .feature(.fontTypography, .iconFixedSize)
                            .view

                        self.model.icons.valueDislayView(
                            self.effectValue.currentRangeScale
                        )
                        .scaleEffect(0.5)
                        self.model.icons.flowDislayView(
                            self.effectValue.flow
                        )
                        .opacity(self.effectValue.flow.doubleValue)
                        .scaleEffect(0.25 + (0.25 * self.effectValue.flow))
                    }
                    .lux
                    .trait(self.model.traitNormal)
                    .feature(.foregroundColor, .fontTypography, .iconFixedSize)
                    .view
                }
            }
            Spacer()
