...
//
//  Licencing info 
//  FileHash: 14117812-AF66-42F7-931C-32CB0C4B1ECF
...
struct ColorPickerControllerUI: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> DynamicColorPickerViewController {
        let colorPickerController = DynamicColorPickerViewController()
        colorPickerController.delegate = context.coordinator
        colorPickerController.useRadialPalette = false

        colorPickerController.view.backgroundColor = .clear

        colorPickerController.addColorControls(mode: mode)
        colorPickerController.selectedColor = color.uiColor()

        model.colorPicker = colorPickerController

        return colorPickerController
    }

    func updateUIViewController(_: DynamicColorPickerViewController, context _: Context) {}

    class Coordinator: NSObject, ColorPickerDelegate {
        var parent: ColorPickerControllerUI
        var observer: AnyCancellable?

        init(_ colorPicker: ColorPickerControllerUI) {
            parent = colorPicker
        }

        func colorPicker(_: ColorPickerController, selectedColor: UIColor, usingControl _: ColorControl) {
            parent.model.editingColorBinding.wrappedValue = selectedColor.uiColor
//            NotificationCenter.default.post(name: .ColorPickerSelectedColor, object: nil, userInfo: ["color": selectedColor])
        }

        func colorPicker(_: ColorPickerController, confirmedColor: UIColor, usingControl _: ColorControl) {
            parent.model.editingColorBinding.wrappedValue = confirmedColor.uiColor
//            NotificationCenter.default.post(name: .ColorPickerConfirmedColor, object: nil, userInfo: ["color": confirmedColor])
        }
    }
}
