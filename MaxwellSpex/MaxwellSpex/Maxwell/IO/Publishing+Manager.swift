...
//
//  Licencing info 
//  FileHash: 6CE7F1AB-7442-4D18-85BD-76A6E6432D9E
...
extension PublishingManager {
    struct State: Codable {
        var publications: [String: [Publication]] = [:]
    }
}

class PublishingManager: ObservableObject, StorableItem {
...
extension PublishingManager {
    static let storageDB = Storage.Database<State>(table: storageTable) { storageInitializer() }
    static var storageTable: Storage.Table {
        Storage.Table(name: "Publishing")
    }

    static var storageInitializer: () -> State {
        { State() }
    }

    static func persist(look: Look, assetURLString: String) -> Publication? {
        let publication = Publication(
            look: look.clone(),
            assetURLString: assetURLString
        )
        shared.state.append(publication)
        return publication
    }
}
...
extension PublishingManager {
    func storeDatabase() {
        Self.storageDB.store(item: state)
        publishToFireStorage()
    }

    func restoreDatabase() {
        state = Self.storageDB.restore()
    }
}

extension PublishingManager {
    func publishToFireStorage() {
        let feed = state.publicFeed()
        FireStorage.updatePublic(table: feed.publicContainer, childValues: feed.publicData)
    }
}

extension PublishingManager.State {
    func publications(for look: Look) -> [Publication] {
        publications[look.meta.creationId] ?? []
    }

    func latestPublication(look: Look) -> Publication? {
        publications(for: look).sorted(by: { $0.look.meta.publishedVersion > $1.look.meta.publishedVersion }).first
    }

    func latestPublication(lookCreationId: String) -> Publication? {
        let source = publications[lookCreationId] ?? []
        return source.sorted(by: { $0.look.meta.publishedVersion > $1.look.meta.publishedVersion }).first
    }

    mutating func append(_ publication: Publication) {
        let look = publication.look
        var current = publications[look.meta.creationId] ?? []
        current.insert(publication, at: 0)
        publications[look.meta.creationId] = current
    }

    mutating func remove(at index: Int, from publication: Publication?) {
...
extension PublishingManager.Feed {
    var publicContainer: String {
        "lux"
    }

    var publicData: [String: String] {
        publications.reduce([:]) { aresult, publication in
            var result = aresult
            result[publication.publicId] = publication.asJSONString()
            return result
        }
    }
}
...
extension PublishingManager.State {
    var allKeys: [String] {
        Array(publications.keys)
    }

    func latestPublications() -> [Publication] {
        let latests = allKeys.map { latestPublication(lookCreationId: $0) }
        return latests.filter { $0 != nil } as? [Publication] ?? []
    }

    func publicFeed() -> PublishingManager.Feed {
        PublishingManager.Feed(publications: latestPublications())
    }
}

extension Publication {
    var publicId: String {
        look.publishPublicShortId
    }
}
