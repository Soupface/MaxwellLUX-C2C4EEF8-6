...
//
//  Licencing info 
//  FileHash: C4AA72EB-1982-493D-815E-C0A649D3757D
...
struct ImportView: View {
    ...
    var trait: Trait {
        model.look?.trait ?? Trait()
    }

    @State var message = "New Lux"
    ...
    var body: some View {
        ...
        .lux
        .if(self.model.look != nil) { $0.feature(.backgroundSystemBlur) }
        .view
    }
    ...
    func content() -> some View {
        Column {
            Group {
                LookDisplay(look: self.model.look ?? Look())
                    .frame(maxHeight: 123)
                Row {
                    Spacer()
                    HStack {
                        Text("\(self.model.look?.publishName ?? "Lux")")
                            .bold()
                        Text(" is ready")
                    }
                    .lux
                ...
                Row {
                    ...
                    Button(action: self.cancel) {
                        Text("Dismiss")
                            ...
                            .style(.icon, .buttonLarge, .paragraph)
                            .view
                    }
                    Button(action: self.apply) {
                        ...
                        .lux
                        .trait(self.trait)
                        .tweak(.headlineLayout, .activeSurface)
                        .style(.icon, .buttonLarge, .paragraph)
                        .view
                    }
                }
