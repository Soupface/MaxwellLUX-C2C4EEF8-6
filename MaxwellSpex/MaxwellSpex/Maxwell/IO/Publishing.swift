...
//
//  Licencing info 
//  FileHash: 544C7F63-8E87-484E-B2E2-8A8F2B956521
...
class PublishingModel: ObservableObject {
    @Published var state = PublishingState()

    var importer: ImportModel?
}

extension PublishingModel {
    func publish(look: Look, completion: @escaping (Look?) -> Void) {
        // save cloudinary doc -> obtain asset url
        // create short-asset-url
        // formulate qr code with short-asset-url -> upload and obtain preview
        // formulate short url with assetURL + coverURL + fallbackURL

        var assetCDNURL: URL?
        var assetShortURL: URL?
        var webDynamicURL: URL?
        var QRLUXURL: URL?
        var finalURL: URL?
        var publishedLook: Look?

        func start() {
            state.publicationIdx = nil
        ...
        func finish() {
            guard let publishedLook = publishedLook else {
                fail()
                return
            }

            guard let assetShortURL = assetShortURL else {
                fail()
                return
            }

            state.publication = PublishingManager.persist(
                look: publishedLook,
                assetURLString: assetShortURL.absoluteString
            )

            assert(look.meta.creationId == state.publication?.look.meta.creationId)

            state.isPublishing = false
            completion(publishedLook)
        }
        ...
        func postAsset() {
            Uploader.post(look) { urlstring in
                ...
                guard let url = URL.optional(from: urlstring) else {
                    fail()
                    return
                }
        ...
        func postShortAssetURL() {
            guard let assetURL = assetCDNURL else {
                fail()
                return
            }

            FireLink.dynamicShortLink(assetURL) { url in
                assetShortURL = url
                resolve()
            }
        }
        ...
        func postCover() {
            ...
            LUXQR.QRLux(from: look, urlString: assetShortURL.absoluteString) { image in
                guard let image = image else {
                    fail()
                    return
                }
                Sharing.saveToLibrary(image)

                Uploader.post(QRLux: image, publicId: look.publishPublicId) { coverURLString in
                    guard let coverURLString = coverURLString else {
                        fail()
                        return
                    }

                    guard let url = URL.optional(from: coverURLString) else {
                        fail()
                        return
                    }
        ...
        func publishLook() {
            guard
                let assetShortURL = assetShortURL,
                let fallbackURL = webDynamicURL,
                let qrURL = QRLUXURL,
                let finalURL = finalURL
            else { fail(); return }

            publishedLook = look.publishedVersion(assetURL: assetShortURL, webURL: fallbackURL, qrURL: qrURL, shortURL: finalURL)
            resolve()
        }

        func prepareFallback() {
            guard let qrURL = QRLUXURL else {
                fail()
                return
            }
            let data: String = qrURL.absoluteString.base64Encoded()
            let ref: String = "https://luxyfy.me/qr?ref=\(data)"
            guard let url = URL.optional(from: ref) else {
                fail()
                return
            }
            webDynamicURL = url
        ...
        func createShortLink() {
            guard
                let assetURL = assetCDNURL,
                let qrURL = QRLUXURL,
                let webURL = webDynamicURL
            else {
                fail()
                return
            }

            FireLink.dynamicQRLuxLink(look: look, assetURL: assetURL, qrURL: qrURL, webURL: webURL) { url in
                finalURL = url
                resolve()
            }
        }
        ...
        func resolve() {
            guard assetCDNURL != nil else {
                print("publishing -> ASSETURL")
                postAsset()
                return
            }

            guard assetShortURL != nil else {
                print("publishing -> ASSETURL")
                postShortAssetURL()
                return
            }

            guard QRLUXURL != nil else {
                print("publishing -> QRURL")
                postCover()
                return
            }

            guard webDynamicURL != nil else {
                print("publishing -> fallbackURL")
                prepareFallback()
                return
            }

            guard finalURL != nil else {
                print("publishing -> shortURL")
                createShortLink()
                return
            }

            guard publishedLook != nil else {
                print("publishing -> publishedLook")
                publishLook()
                return
            }

            print("publishing -> finalizing!!")
            finish()
        }
...
extension PublishingModel {
    func copyJSONData() {
        ...
        let data = look.asJSONString()
        UIPasteboard.general.string = data
        BannerModel.shared.push(BannerModel.Banner(message: "Copied"))
    }

    func saveToFile() {
        let look = Look.current
        guard
            let fileURL = FileSystem.writeTemp(string: look.asJSONString(), fileName: look.publishFileNameWithExt) else {
            assert(false)
            BannerModel.shared.push(BannerModel.Banner(message: "Please Retry"))
            return
        }

        FileSystem.save(fileAt: fileURL) { _ in
