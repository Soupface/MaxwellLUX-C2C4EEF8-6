...
//
//  Licencing info 
//  FileHash: 28D61454-101F-46FD-9F34-19683ABE5081
...
class Sharing {
    static var onWillShowActivitiesSheet: (() -> Void)?
    static func activitiesFor(string: String, completion: @escaping () -> Void) {
        let items = [string]

        DispatchQueue.main.async {
            onWillShowActivitiesSheet?()
            DispatchQueue.main.async {
                let ac = UIActivityViewController(activityItems: items, applicationActivities: nil)
                UIApplication.present(ac) {
                    completion()
                }
            }
        }
    ...
    static func activitiesFor(any: Any, completion: @escaping () -> Void) {
        let items = [any]

        DispatchQueue.main.async {
            onWillShowActivitiesSheet?() // self.sheet = nil
    ...
    // refactor this
    static func activitiesFor(fileURL: URL, string: String, completion: @escaping () -> Void) {
        do {
        ...
        activitiesFor(any: items, completion: completion)
    }
}
...
extension Sharing {
    ...
    static func activitiesFor(look: Look, image: UIImage, completion: @escaping () -> Void) {
        guard let data = image.pngData() else {
            return completion()
        }

        let temporaryFileURL = look.QRLuxTempFileURL
        do {
        ...
        DispatchQueue.main.async {
            self.onWillShowActivitiesSheet?() // self.sheet = nil
...
extension Sharing {
    static func copyShortURL(look: Look, completion: @escaping () -> Void) {
        guard let link = look.meta.shortURL else {
            assert(false)
            completion()
            return
        }
        ...
        BannerModel.shared.push(BannerModel.Banner(message: "QRLux copied"))
    }

    static func saveToLibrary(_ image: UIImage) {
        PHPhotoLibrary.shared().savePhoto(image: image, albumName: albumName)
    }

    static func copyQRLuxImage(look: Look) {
        guard let assetURL = look.meta.assetURL else {
            assert(false)
            return
        }

        LUXQR.QRLux(from: look, urlString: assetURL) { image in
    ...
    static func saveQRLuxImageToFile(look: Look) {
        guard let assetURL = look.meta.assetURL else {
            assert(false)
            return
        }
        ...
        LUXQR.QRLux(from: look, urlString: assetURL) { image in
            ...
            FileSystem.savePNG(image: image, name: "\(look.publishFileName).png") { _ in
                BannerModel.shared.push(BannerModel.Banner(message: "Saved"))
            }
        }
    }

    static func saveQRLuxImageToLibrary(look: Look) {
        guard let assetURL = look.meta.assetURL else {
            assert(false)
            return
        }
        ...
        LUXQR.QRLux(from: look, urlString: assetURL) { image in
            ...
            saveToLibrary(image)
            BannerModel.shared.push(BannerModel.Banner(message: "Saved to Library"))
        }
    }
}
