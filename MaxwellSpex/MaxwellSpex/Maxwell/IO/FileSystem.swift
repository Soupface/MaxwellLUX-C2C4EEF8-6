...
//
//  Licencing info 
//  FileHash: AF9A8638-610A-4C77-ADF7-08BEDB3BEB2A
...
class FileSystem: NSObject {
    static let LUXFileType = "com.maxwelldesign.document.lux"
    typealias Completion = (URL?) -> Void
    var loadCompletion: Completion?

    static func save(fileAt url: URL, name: String = String(UUID().uuidString.prefix(8)), completion: @escaping Completion) {
        guard shared.loadCompletion == nil else {
            assert(false)
            return
        }

        shared.loadCompletion = completion

        _save(fileAt: url, name: name, completion: completion)
    }

    static func _save(fileAt url: URL, name: String = String(UUID().uuidString.prefix(8)), completion: @escaping Completion) {
        guard url.isFileURL else {
            save(fileAtRemote: url, name: name, completion: completion)
            return
        }

        let controller = UIDocumentPickerViewController(url: url, in: UIDocumentPickerMode.exportToService)
        controller.delegate = FileSystem.shared
        UIApplication.present(controller)
    }

    static func load(documentTypes: [String] = [FileSystem.LUXFileType], completion: @escaping Completion) {
        guard shared.loadCompletion == nil else {
            assert(false)
            return
        }
...
extension FileSystem {
    static func save(fileAtRemote url: URL, name: String = String(UUID().uuidString.prefix(8)), completion: @escaping Completion) {
        Download.from(url: url, display: true) { data in
            ...
            let tmpFileURL = Self.tempFolder.appendingPathComponent(name)
            ...
            _save(fileAt: tmpFileURL, name: name, completion: completion)
        }
    }
}

extension FileSystem: UIDocumentPickerDelegate {
    static let shared = FileSystem()

    func documentPicker(_ controller: UIDocumentPickerViewController, didPickDocumentsAt urls: [URL]) {
        //        if controller.documentPickerMode == UIDocumentPickerMode.import {
        guard let url = urls.last else { return }
        loadCompletion?(url as URL)
        UIApplication.rootVC.dismiss(animated: true)
        loadCompletion = nil
        //        }
        let message = controller.documentPickerMode == UIDocumentPickerMode.import ? "Imported" : "Saved"
        BannerModel.shared.push(BannerModel.Banner(message: message))
    }
    ...
    func documentPickerWasCancelled(_: UIDocumentPickerViewController) {
        ...
        UIApplication.rootVC.dismiss(animated: true)
        loadCompletion = nil
        BannerModel.shared.push(BannerModel.Banner(message: "Canceled"))
    }
}

extension FileSystem {
    static let tempFolder = tempFolderURL()

    static var fileManager: FileManager {
        FileManager.default
    }

    static func tempFolderURL() -> URL {
        let temporaryDirectoryURL = URL(fileURLWithPath: NSTemporaryDirectory(),
                                        isDirectory: true)

        let temporaryFolder = "MaxwellLUX-\(UUID().uuidString.prefix(8))"
        let temporaryFolderURL = temporaryDirectoryURL.appendingPathComponent(temporaryFolder)
        ...
        assert(fileManager.fileExists(atPath: temporaryFolderURL.path, isDirectory: &isDirectory))
        assert(isDirectory.boolValue == true)
...
extension FileSystem {
    static func writeTemp(string: String, fileName: String = UUID().uuidString) -> URL? {
    ...
    static func writeTempPNG(image: UIImage, fileName: String = UUID().uuidString) -> URL? {
    ...
    static func writeTempJPG(image: UIImage, quality: CGFloat = 0.75, fileName: String = UUID().uuidString) -> URL? {
    ...
    static func writeTemp(data: Data, fileName: String) -> URL? {
...
extension FileSystem {
    static func writeUserDocument(string: String, fileName: String) -> URL? {
        if let data = string.data(using: .utf8) {
            return writeTemp(data: data, fileName: fileName)
        }
        return nil
    ...
    static func writeUserDocumentPNG(image: UIImage, fileName: String) -> URL? {
        if let data = image.pngData() {
            return writeTemp(data: data, fileName: fileName)
        }
        return nil
    ...
    static func writeUserDocumentJPG(image: UIImage, quality: CGFloat = 0.75, fileName: String) -> URL? {
        if let data = image.jpegData(compressionQuality: quality) {
            return writeTemp(data: data, fileName: fileName)
        }
        return nil
    ...
    static func writeUserDocument(data: Data, fileName: String) -> URL? {
...
extension FileSystem {
    static func savePNG(image: UIImage, name: String = String(UUID().uuidString.prefix(8)), completion: @escaping Completion) {
