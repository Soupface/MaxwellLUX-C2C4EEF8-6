...
//
//  Licencing info 
//  FileHash: EF214478-D852-4667-8488-5C89FBDF4479
...
class Download: ObservableObject {
    static let queue = DispatchQueue(label: "com.maxwelldesign.download")
    static let lock = NSLock()

    typealias DataCompletion = (Data?) -> Void
    typealias FileCompletion = (URL?) -> Void
    static let shared = Download()
    @Published var tasksCount = 0
    @Published var displayTasksCount = 0
...
extension Download {
    static func from(url: URL, fileName: String, display: Bool = false, completion: @escaping FileCompletion) {
        from(url: url, display: display) { data in

            guard
                let data = data,
                let url = FileSystem.writeTemp(data: data, fileName: fileName) else {
                assert(false)
                completion(nil)
                return
            }
...
extension Download {
    static func from(url: URL, display: Bool = false, completion: @escaping DataCompletion) {
        beingTask(display: display)

        AF.request(url).responseData { response in

            finishTask(display: display)
...
//                assert(false)
    ...
    static func beingTask(display: Bool) {
        queue.async {
            lock.lock()
            DispatchQueue.main.async {
                shared.tasksCount += 1
                if display { shared.displayTasksCount += 1 }
            }
    ...
    static func finishTask(display: Bool) {
        queue.async {
            lock.lock()
            DispatchQueue.main.async {
                shared.tasksCount -= 1
