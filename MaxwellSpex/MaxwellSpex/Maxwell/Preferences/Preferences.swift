...
//
//  Licencing info 
//  FileHash: C1FAF836-73C4-4EB5-B4D8-9295973DBE92
...
//  MaxwellSpex
//
//  Created by mark maxwell on 5/17/20.
...
class Preferences: ObservableObject {
    ...
    static let MaxColors = 99
    struct State: Codable {
        enum CodingKeys: String, CodingKey {
            case dismissedImports
            case recentColors
            case initialAppsection
            //todo add recentcolors
        }

        var _recentColors: [Color] = []
        var ignoredImportSuggestions: [String] = []
        var initialAppsection: AppModel.Mode = .store
        ...
        var recentColors: [Color] {
            ...
            set {
                _recentColors = newValue.removingDuplicates().suffix(Preferences.MaxColors)
            }
        }

        var codableRecentColors: [CodableColor] {
            recentColors.map { CodableColor($0) }
        }

        public func encode(to encoder: Encoder) throws {
            var container = encoder.container(keyedBy: CodingKeys.self)

            try container.encode(codableRecentColors, forKey: .recentColors)
            try container.encode(ignoredImportSuggestions, forKey: .dismissedImports)
            try container.encode(initialAppsection.rawValue, forKey: .initialAppsection)
        }
        ...
        public init(from decoder: Decoder) throws {
            ...
            ignoredImportSuggestions = try container.decode([String].self, forKey: .dismissedImports)

            recentColors = try container.decode([CodableColor].self, forKey: .recentColors).map { $0.uiColor.uiColor }

            let mode = try? container.decode(String.self, forKey: .initialAppsection)
            initialAppsection = AppModel.Mode(rawValue: mode ?? "") ?? .lux
        }
    }

    var state: State {
        didSet {
            Preferences.storageDB.store(item: state)
        }
    }

    init() {
        state = Preferences.storageDB.restore()
    }
}

extension Preferences: StorableItem {
    static let storageDB = Storage.Database<Preferences.State>(table: storageTable) { storageInitializer() }
    static var storageTable: Storage.Table {
        Storage.Table(name: "Preferences")
    }

    static var storageInitializer: () -> Preferences.State {
        { Preferences.State() }
    }
}
