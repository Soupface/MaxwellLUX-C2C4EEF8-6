...
//
//  Licencing info 
//  FileHash: 883330A1-8827-480B-8070-DBF06C25F998
...
struct LoginView: View {
    ...
    var showActionSheet: Binding<Bool> {
        Binding<Bool>(
            ...
            $0 == false ? self.mode = .normal : nil
        }
    }

    @State var focus: CGFloat = 1.0
    @State var mode: Mode = .normal

    var player: AVPlayer {
        let player = VideoPlayer.player(for: StoreModel.Product.maxwellYourWayVideo.videoConfig)
        player.isMuted = true
        return player
    }
    ...
    func signIn() {
        ATracker.track(.login(.tapStart))
        FireAuth.appleSignIn { result in
            result != nil ? ATracker.track(.login(.didComplete)) : ATracker.track(.login(.didFail))
        }
    }

    func showLearnMore() {
        ATracker.track(.login(.tapLearnMore))
        mode = .login
    }

    func showTOS() {
        ATracker.track(.login(.tapTOS))
        mode = .tos
    }
    ...
    func reFocus() {
        withAnimation(.easeIn(duration: 3.69)) {
            focus = CGFloat([1.0, 0.0].randomElement()!)
        }
    ...
    var browser: WebViewStore {
        mode == .login ? navigator.applogin : navigator.privacy
    }
    ...
    var body: some View {
        Column {
            ...
            Column {
                ...
                Button(action: self.showLearnMore) {
                    Text("Learn More")
                        .lux
                        .tweak(.captionLayout)
                        .tweak(.canvasSurface)
                        .feature(.flexibleWidth)
                        .style(.buttonLarge)
                        .feature(.backgroundSystemBlur, .cornerRadius, .secondaryOpacity)
                        .style(.paragraph)
                        .view
                }

                Row {
                    Button(action: self.showTOS) {
                        Text("Privacy & TOS")
                            .lux
                            .tweak(.footerLayout)
                            .style(.paragraph)
        ...
        .onAppear {
            self.reFocus()
            self.navigator.applogin.loadIfNeeded()
            self.navigator.privacy.loadIfNeeded()
        }
        .sheet(isPresented: self.showActionSheet) {
            ActionSheetContainer(dismiss: { self.mode = .normal }) {
                ZStack {
                    Column {
                        Spacer()
                        ActivityIndicator()
                    ...
                    BrowserView(webViewStore: self.browser)
                        .onAppear {
                            self.browser.loadIfNeeded()
                        }
                }
                .lux
                .style(.panel)
