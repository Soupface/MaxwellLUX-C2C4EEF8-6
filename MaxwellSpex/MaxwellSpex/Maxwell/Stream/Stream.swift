...
//
//  Licencing info 
//  FileHash: 23465B7D-F341-4DBF-9966-B03C9BBC7161
...
class Stream: ObservableObject {
    ...
    @Published var preferredLook: Look?
    @Published var tunedLook: Look?
    @Published var streamNames: [String] = []

    var multiPeer: MultiPeer?
    var currenLookObserver: AnyCancellable?
...
extension Stream {
    func commitConnection() {
        ...
        serverNameIsSet ? broadcastLook() : nil
    }
...
extension Stream {
    func broadcastLook() {
        if let preferredLook = preferredLook {
            return stream(Photon(serverName: deviceName, look: preferredLook))
        }
        stream(Photon(serverName: deviceName, look: Look.current))
    }
}
...
extension Stream: MultiPeerDelegate {
    func multiPeer(didReceiveData data: Data, ofType type: UInt32) {
        ...
        case Photon.DataType.string.rawValue:
            ...
            if let photon: Photon = Codec.object(fromJSON: stringData) {
                return tune(photon)
            }
    ...
    func multiPeer(connectedDevicesChanged names: [String]) {
        streamNames = names
        updateStatus()
    }
}
