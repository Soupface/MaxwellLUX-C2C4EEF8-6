...
//
//  Licencing info 
//  FileHash: 7A9C9D1E-8557-48C7-9617-3E6092544DAA
...
struct StreamsView: View {
    ...
    var helpText: String {
        ...
        case .tuning:
            return "Tune for nearby streams and recieve Lux tweaks in realtime"
        case .streaming:
            return "Stream to nearby tuners and share Lux tweaks in realtime"
        }
    }

    var body: some View {
        Column {
            StreamViewHeader(model: self.model, stream: self.stream)

            Column {
                Group {
                    self.stream.config.shouldConnect ?
                        Group {
                            StreamViewConnectedContent(model: self.model, stream: self.stream)
                        }.lux.view
                        :
                        VStack {
                            Spacer()
                ...
                self.stream.config.role != nil ?
                    StreamPlayButton(model: self.model, stream: self.stream)
                    .lux
                    .style(.layoutElement)
                    .view
                    : nil
            ...
            StreamViewTabs(model: self.model, stream: self.stream)
        }
        .lux
        .style(.panel)
        .view
...
struct StreamsViewStreamsList: View {
    @ObservedObject var stream: Stream

    func edit(_: LookDisplayInfo) {}

    func more(_: LookDisplayInfo) {}

    func select(server name: String) {
        ATracker.track(.stream(.tapServer))
...
struct StreamsViewThemesList: View {
    @ObservedObject var model: AppModel
    ...
    var body: some View {
        List {
            ForEach(model.state.looks, id: \.self) { (info: LookDisplayInfo) in
                LookDisplay(
                    look: info.look,
                    active: self.model.isEditing(info: info),
                    onEdit: {
                        self.edit(info)
                    }
                )
...
struct StreamViewHeader: View {
    @ObservedObject var model: AppModel
    @ObservedObject var stream: Stream

    func configRole(_ role: Stream.Role?) {
...
struct StreamViewConnectedContent: View {
    @ObservedObject var model: AppModel
    @ObservedObject var stream: Stream
            ...
            self.stream.serverNameIsSet ?
                Group {
                    ...
                    Row {
                        ...
                        Button(action: { self.stream.config.serverName = nil }) {
                            HStack {
                                Text("Streams")
...
struct StreamViewTabs: View {
    ...
    func configRole(_ role: Stream.Role?) {
        switch role {
        case .none:
            ATracker.track(.stream(.tapOff))
        case .streaming:
            ATracker.track(.stream(.tapStream))
        case .tuning:
            ATracker.track(.stream(.tapTune))
        }
...
struct StreamPlayButton: View {
    ...
    func toggleConnection() {
        ATracker.track(.stream(.tapPlay))
        stream.config.serverName = nil
