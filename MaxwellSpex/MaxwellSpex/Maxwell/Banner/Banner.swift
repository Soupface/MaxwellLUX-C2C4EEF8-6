...
//
//  Licencing info 
//  FileHash: 4AE607EE-2F7A-4389-94B7-A4559959422C
...
extension BannerModel {
    ...
    static let shared = BannerModel()
    struct Banner: Hashable {
        static func == (lhs: BannerModel.Banner, rhs: BannerModel.Banner) -> Bool {
            lhs.id == rhs.id
        }
        ...
        var message: String
        var duration: CFAbsoluteTime = 3
        var action: BannerAction?
    }
}

class BannerModel: ObservableObject {
    let dispatchQueue = DispatchQueue(label: "bannerQueue")

    @Published var queue: [Banner] = []
    @Published var shown = false

    var displayTimer: Timer?
    var hideTimer: Timer?

    func autoShowHide(duration: CFAbsoluteTime) {
        dispatchQueue.async {
            DispatchQueue.main.async {
                ...
                let timer1 = Timer(timeInterval: duration + 0.500, repeats: false) { time in
                    time.invalidate()
                    self.dismiss()
                }

                let timer2 = Timer(timeInterval: duration, repeats: false) { time in
                    time.invalidate()
                    self.shown = false
                }
                RunLoop.current.add(timer1, forMode: RunLoop.Mode.common)
                RunLoop.current.add(timer2, forMode: RunLoop.Mode.common)
                self.displayTimer = timer1
                self.hideTimer = timer2
            }
        }
    }
    ...
    func removeTopBanner() {
        if let banner = queue.popLast() {
            banner.action?()
        }
    }
    ...
    func push(_ banner: Banner) {
        queue.append(banner)
        autoShowHide(duration: banner.duration)
    }

    func push(message: String?, duration: CFAbsoluteTime = 3) {
        guard let message = message else { return }
        push(BannerModel.Banner(message: message, duration: duration))
    }
}

struct BannerTopView: View {
    var body: some View {
        Column {
            Spacer()
            Row {
...
struct BannerView: View {
    @ObservedObject var model = BannerModel.shared

    var current: BannerModel.Banner? {
        model.queue.last
    }
    ...
    var body: some View {
        current != nil ?
            ...
            .style(.paragraph)
            .view
            .background(self.lux.draw.roundedRectangle
                .fill(self.lux.spec.surface.active))
            .scaleEffect(self.model.shown ? 1 : 0)
            .opacity(self.model.shown ? 1 : 0)
            .animation(.spring())
