...
//
//  Licencing info 
//  FileHash: 14A7B516-70A0-40F6-96B1-C49E42158A7A
...
struct IntroView: View {
    ...
    var data: [Data] {
        [
            Data(text: "LET", color: .white, foreground: .black),

            Data(text: "THERE", color: .black, foreground: .white),

            Data(text: "BE", color: .white, foreground: .black),

            Data(text: "LUX", color: .black, foreground: .white),
        ]
    }

    func next() {
        index = min(index + 1, data.count - 1)
    }
        ...
        ZStack {
            Column {
                Text(self.currentData.text)
                    ...
                    .foregroundColor(self.currentData.foreground)
                    .animation(.easeInOut)
            }
            .lux
            // .if(userInterfaceIdiom: .phone) { $0.feature(.screenFrame) }
            .view
            .background(self.currentData.color)
            .onTapGesture {
                self.next()
            }
