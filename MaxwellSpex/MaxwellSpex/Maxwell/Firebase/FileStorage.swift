...
//
//  Licencing info 
//  FileHash: 894DA56A-FE0C-497E-B6B6-05C3BCC489C3
...
extension FireStorage {
    static func launchSetup() {
        #if DEBUG
            ref = Database.database().reference().child("dev")
        #else
            ref = Database.database().reference().child("prod")
        #endif
    }
}

extension FireStorage {
    static var storage: DatabaseReference? {
        guard let user = user else {
            return nil
        }
        return ref.child("users").child(user.uid).child("storage")
    }

    static func tableName(key: String) -> String {
        key.alphanumeric.lowercased()
    }

    static func tableNamed(key: String) -> DatabaseReference? {
        storage?.child(tableName(key: key))
    }

    static func update(table: String, data: String) {
        guard let storage = storage else { return }
        storage.child(tableName(key: table)).setValue(["data": data])
    }
}

extension FireStorage {
    static var publicStorage: DatabaseReference? {
        ref.child("public")
    }

    static func updatePublic(table: String, childValues: [String: Any]) {
        guard
            let publicStorage = publicStorage,
            let user = user
        else { return }
        publicStorage.child(tableName(key: table)).child(user.uid).updateChildValues(childValues)
    }
}

extension FireStorage {
    static var licensingRef: DatabaseReference? {
        guard let user = user else {
            return nil
        }
        return ref.child("user").child(user.uid).child("licensing").child("v1")
    }

    static func restoreLicensing(completion: @escaping (LicensingModel.License?) -> Void) {
        licensingRef?.observeSingleEvent(of: .value, with: { snapshot in
            guard
                let jsonString = snapshot.value as? String
            else {
                completion(nil)
                return
            }

            guard let license: LicensingModel.License? = Codec.object(fromJSON: jsonString.base64Decoded()) else {
                completion(nil)
                return
            }
            completion(license)
    ...
    static func storeLicensing(_ license: LicensingModel.License?) {
        licensingRef?.setValue(license?.asJSONString().base64Encoded())
    }
}
