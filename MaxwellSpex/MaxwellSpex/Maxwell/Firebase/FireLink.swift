...
//
//  Licencing info 
//  FileHash: 1D5126A8-19C7-4FBD-AE93-0937E06B43D3
...
extension FireLink {
    typealias FireLinkCompletion = (URL?) -> Void
    static let LinkPrefix = FireConfig.env.linkPrefix
    static let ShortLinkPrefix = FireConfig.env.shortLinkPrefix

    static func dynamicShortLink(_ longURL: URL, completion: @escaping FireLinkCompletion) {
        let fail = {
            assert(false)
            completion(nil)
        }

        guard let linkBuilder = DynamicLinkComponents(link: longURL, domainURIPrefix: ShortLinkPrefix) else {
            fail()
            return
        }

        linkBuilder.analyticsParameters = DynamicLinkGoogleAnalyticsParameters(source: "app",
                                                                               medium: "link",
                                                                               campaign: "qrlux")

        linkBuilder.iTunesConnectParameters = DynamicLinkItunesConnectAnalyticsParameters()
        linkBuilder.iTunesConnectParameters?.providerToken = "app"
        linkBuilder.iTunesConnectParameters?.campaignToken = "link"
    ...
    static func dynamicQRLuxLink(look: Look, assetURL: URL, qrURL: URL, webURL _: URL, completion: @escaping FireLinkCompletion) {
        let fail = {
            assert(false)
            completion(nil)
        }

        guard let linkBuilder = DynamicLinkComponents(link: assetURL, domainURIPrefix: LinkPrefix) else {
            fail()
            return
        }

        linkBuilder.iOSParameters = DynamicLinkIOSParameters(bundleID: "com.maxwell.spex")
        ...
        linkBuilder.iTunesConnectParameters = DynamicLinkItunesConnectAnalyticsParameters()
        linkBuilder.iTunesConnectParameters?.providerToken = "app"
        linkBuilder.iTunesConnectParameters?.campaignToken = "qrlux"

        linkBuilder.socialMetaTagParameters = DynamicLinkSocialMetaTagParameters()
        linkBuilder.socialMetaTagParameters?.title = "\(look.name) LUX"
        linkBuilder.socialMetaTagParameters?.descriptionText = "\(look.displayLemma)"
        linkBuilder.socialMetaTagParameters?.imageURL = qrURL
