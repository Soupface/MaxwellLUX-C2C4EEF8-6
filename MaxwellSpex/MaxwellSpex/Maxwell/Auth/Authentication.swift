...
//
//  Licencing info 
//  FileHash: F1F2B940-3A3A-4DC3-8578-7E4857D896A4
...
extension Authentication: StorableItem {
    static let storageDB = Storage.Database<String>(table: storageTable) { storageInitializer() }
    static var storageTable: Storage.Table {
        Storage.Table(name: "installationIdKey")
    }

    static var storageInitializer: () -> String {
        { String(UUID().uuidString.replacingOccurrences(of: "-", with: "").prefix(8)) }
    }

    static var installationId: String {
        Self.storageDB.restore()
    }
}
...
extension Authentication {
    ...
    static var hasSignedUser: Bool {
        true
//        user != nil && user?.isAnonymous == false
    }

    static var user: User? {
        shared.state.user ?? Auth.auth().currentUser
    }

    static var userId: String {
        user?.uid ?? installationId
    }
}

extension Authentication {
    static func syncProviders() {
        Permissions.requestSignInPermissions()
