...
//
//  Licencing info 
//  FileHash: 701AE03A-3A3D-4C3D-AD88-6B4B8A74CD79
...
//  MaxwellSpex
//
//  Created by Mark C. Maxwell on on The New Lux
...
extension LUXQR {
    typealias QRLuxCompletion = (UIImage?) -> Void
}
...
class LUXQR {
    ...
    static func scanImage(image: CIImage) -> String? {
    ...
    static func _QRLux(from look: Look, block: @escaping QRLuxCompletion) {
        Uploader.post(look) { urlString in

            let snapshot = Lux.Snapshot(size) { _ in QRLuxLook(look: look, url: urlString).frame(width: size.width, height: size.height).lux.view }

            let capture = snapshot.capture()

            let ciImage = CIImage(image: capture)
            let test = scanImage(image: ciImage!)
            assert(test == urlString)
            block(capture)
        }
    }

    static func QRLux(from look: Look, urlString: String, block: @escaping QRLuxCompletion) {
        DispatchQueue.main.async {
            let snapshot = Lux.Snapshot(size) { _ in QRLuxLook(look: look, url: urlString).lux.view }
    ...
    static func QRLux(from string: String, color: UIColor, imageWidth: CGFloat = 720) -> UIImage {
        let data = Data(string.utf8)
        filter.setValue(data, forKey: "inputMessage")

        if let outputImage = filter.outputImage,
            let tintedImage = outputImage.tinted(using: color) {
            let ratio = imageWidth / tintedImage.extent.size.width
            let transform = CGAffineTransform(scaleX: ratio, y: ratio)

            let final = tintedImage.transformed(by: transform)

            if
                let cgimg = context.createCGImage(final, from: final.extent) {
...
struct QRLuxLook: View {
    var look: Look
    var url: String?
    var iconQR = false

    func QRLux(_ color: UIColor) -> Image {
        guard let url = url else {
            return Image(systemName: "xmark")
        }
        return Image(uiImage: LUXQR.QRLux(from: url, color: color))
    }
    ...
    var body: some View {
        GeometryReader { geo in
            ZStack {
                LookDisplay(look: self.look, maxWidth: nil, showIcons: false, animate: false, realtime: false, cornerRadius: false)
                    .frame(width: geo.size.width * 1, height: geo.size.width * 1)

                Column {
                    Text("V\(self.look.meta.publishedVersion)")
                        .lux
                        .tweak(.activeSurface, .darkSpec)
                        .tweak(.captionLayout)
                        .style(.button, .text)
                        .view
                ...
                Column {
                    Spacer()
                    LookDisplay(look: self.lookLux, maxWidth: nil, showIcons: false, animate: false, flipped: true, realtime: false, cornerRadius: false)
                        .frame(width: geo.size.width * 1.0, height: geo.size.width * 0.3)
                        .overlay(
                            ZStack {
                                Group {
                                    self.iconQR ?
                                        ...
                                        .lux.view
                                        :
                                        self.QRLux(Color(white: 0.1).uiColor())
                                        .resizable()
                                        .lux.view
                                }
                                .padding(geo.size.width * 0.0234 / 2.0)
                                .background(
...
struct QRLuxLook_Previews: PreviewProvider {
    static var previews: some View {
        QRLuxLook(look: Look(), url: "hello world")
