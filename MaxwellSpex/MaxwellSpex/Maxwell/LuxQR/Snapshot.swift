...
//
//  Licencing info 
//  FileHash: FB4B4E6E-520D-40DE-945E-B3C54F25C6CC
...
extension Lux {
    class Snapshot {
        var size: CGSize
        var block: ViewBlockClosure

        init(_ size: CGSize, block: @escaping ViewBlockClosure) {
            self.size = size
            self.block = block
        }
        ...
        func capture() -> UIImage {
            ...
            return renderedImage(hosting.view)
        }
        ...
        var canvas: AnyView {
            UIView()
                ...
                .viewBlock { block($0.frame(width: size.width, height: size.height).lux.view) }
                .view
                .edgesIgnoringSafeArea(.all)
                .lux.view
        }

        func renderedImage(_ view: UIView) -> UIImage {
            // rect of capure
