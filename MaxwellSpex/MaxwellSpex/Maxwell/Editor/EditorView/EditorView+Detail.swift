...
//
//  Licencing info 
//  FileHash: 6EEFBFFE-14EE-4133-9D1D-4222A7645D5C
...
struct ActiveComponent: View {
    @ObservedObject var model: EditorModel
    var body: some View {
        ZStack {
            ActiveContent(model: model)

            model.isTuning == false ?
                ActiveMenu(model: model)
                : nil
        }
    }
}

struct ActiveMenu: View {
    @ObservedObject var model: EditorModel
    ...
    func tapNextLayout() {
        ATracker.track(.editor(.tapDefaultLayout))
        model.nextLayout()
    }

    func tapNextSurface() {
        ATracker.track(.editor(.tapDefaultSurface))
        model.nextSurface()
    }
    ...
    var body: some View {
        VStack(alignment: .center) {
            ...
            Spacer()
                .lux
                .feature(.separatorFrame)
                .view
            //            VStack {
            //                ScrollView(.horizontal, showsIndicators: false) {
            HStack {
                Button(action: self.tapNextLayout) {
                    Text("\(self.layoutIcon)")
                        .animation(.none)
                        .lux
                ...
                Spacer()
                Button(action: self.tapNextSurface) {
                    Text("\(self.surfaceIcon)")
                        .bold()
                        .animation(.none)
                        .lux
                        .style(.iconLarge, .text)
            ...
            .frame(maxHeight: self.model.componentsMenuHeight)
            .lux
            .style(.bar)
            .view
        }
        .animation(.easeInOut)
        .offset(y: -self.model.componentsMenuOffset)
        .lux
        .unless(self.model.showControls) { $0.feature(.invisible) }
        .if(self.model.hideControls) { $0.feature(.hidden) }
        .if(self.model.isShowingControlTool) { $0.feature(.hidden) }
        .view
    }
}
...
struct ActiveContent: View {
    ...
    var section: EditorMode.Gallery {
        model.componentDetail?.section ?? .panel
    }

    func onTapCode() {
        model.onSourceCodeTap?(section)
    }
        ...
        VStack {
            ScrollView(.vertical, showsIndicators: false) {
                VStack {
                    ...
                    Group {
                        Row {
                            Text(self.section.rawValue.capitalized)
                                .bold()
                                .lux
                                .tweak(.titleLayout)
                        ...
                        Row {
                            self.model.componentDetail?.builder(self.model.trait)
                                .lux
                                .look(refreshId: "active.content")
                                .view
                                .animation(.easeInOut)
                        }
                        Spacer()
                            .frame(minHeight: self.model.layoutSpacing.double)

                        self.model.showControls ?
                            Group {
                                CodeSampleView(trait: self.model.trait, codeSample: self.section.codeSample, onTapCode: self.onTapCode)
                ...
                .frame(minHeight: UIScreen.main.bounds.height, maxHeight: .greatestFiniteMagnitude)
                .offset(x: 0.0, y: self.model.layoutSpacing * 2.0)
            }.frame(minWidth: UIScreen.main.bounds.width)
        }
...
struct CodeSampleView: View {
    ...
    var tweaked: Trait {
        trait.tweak(.canvasSurface)
    }

    func copy() {
        BannerModel.shared.push(message: "Copied")
        ATracker.track(.editor(.tapCopyCode))
        UIPasteboard.general.string = codeSample
        copied.toggle()
    }

    func tapFullCode() {
        ATracker.track(.editor(.tapViewMoreCode))
        onTapCode?()
    }

    func tapHowToCode() {
        ATracker.track(.editor(.tapHowToCode))
        UIApplication.shared.open(URL.optional(from: FireConfig.env.mediaHowToCodeSamplesLink)!)
    }
    ...
    var body: some View {
        Column {
            ...
            Group {
                ...
                Group {
                    Row {
                        Button(action: self.tapFullCode) {
                            HStack {
                    ...
                    Row {
                        Button(action: self.tapHowToCode) {
                            HStack {
                                Image(systemName: "play")
                                Text("How to run this code?")
                            }
                            .lux
                            .trait(self.tweaked)
                            .feature(.flexibleWidth, .secondaryOpacity)
                            .style(.buttonLarge)
                            .view
                        }
                    }
