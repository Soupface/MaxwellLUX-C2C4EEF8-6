...
//
//  Licencing info 
//  FileHash: 5693375D-2D5C-48B0-B04E-8E313BDD470D
...
struct GalleryView: View {
    ...
    func cells(section: EditorMode.Gallery) -> [ContentCell<AnyView>] {
        let contentCell = ContentCell(.content, model: model, section: section, salt: "\(section):content") { _, _ in

            Specimen(model: self.model, section: section)
                .lux.view
        }

        let footer = ContentCell(.spacer, model: model, section: section, salt: "spacer") { _, _ in

            Spacer().frame(minHeight: UIScreen.main.bounds.size.height * 0.5).lux.view
        }

        if model.isShowingControlSurfacePreview {
            return [contentCell, footer]
        }

        return [
            ContentCell(.header, model: self.model, section: section, salt: self.model.title) { _, _ in

                Column(alignment: .leading) {
                    Spacer().frame(minHeight: self.model.layoutSpacing * 3.0)
                    LookDisplay(look: self.model.look,
                                salt: "\(self.model.look.name)\(self.model.look.hashValue)",
                                height: 123,
                                onEdit: {
            ...
            ContentCell(.footer, model: self.model, section: section, salt: "signature") { _, _ in

                Column {
                    Spacer()
                        .frame(minHeight: self.model.layoutSpacing * 3.0)
                    Row {
                        Spacer()
                        Text(self.model.look.displayLinkName)
                            .bold()
                            .lux
                            .tweak(.footerLayout)
                            .style(.iconLarge)
                    ...
                    .view
                    .onTapGesture {
                        UIApplication.shared.open(self.model.look.displayLinkURL)
                    }
                }.lux.view
    ...
    func listId(section: EditorMode.Gallery) -> String {
        "com.maxwell.list.\(section.displayName).\(model.controlPreviewSurface?.rawValue ?? "base.\(model.layoutSpacing)")"
    }
        ...
        NavigationView {
            Form {
                ForEach(model.sections, id: \.self) { section in
                    ...
                    section == self.model.galleryMode ?

                        List {
                            VStack(spacing: 0) {
                                ForEach(self.cells(section: section), id: \.salt) { cell in
                                    cell
                                        .animation(.easeInOut)
                                }
                                Spacer()
                            ...
                            .lux
                            .feature(.verticallyFixed)
                            .view
                        }
                        .animation(.easeInOut)
                        .lux
                        //                    .feature(.backgroundSurfaceComposition)
                        .view
...
struct ContentCell<Content: View>: View {
    typealias ContentClosure = (EditorModel, EditorMode.Gallery) -> Content
    ...
    public init(_ type: CellType, model: EditorModel, section: EditorMode.Gallery, salt: String = "", @ViewBuilder content: @escaping ContentClosure) {
    ...
    var body: some View {
        Column {
            self.content(self.model, self.section)
        }
    }
}
