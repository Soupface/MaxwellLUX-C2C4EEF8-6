...
//
//  Licencing info 
//  FileHash: 9EAEF001-2A0C-4478-9CD1-7FFCF49A5C8D
...
struct NavigationMenu: View {
    ...
    var modeIcon: String {
        model.isDark ? "moon" : "sun.max"
    }

    var labelIcon: String {
        switch model.look.scheme {
        case .light:
            switch model.look.color.priority {
            case .primary:
                return "1.square"
            case .secondary:
                return "2.square"
    ...
    var fontIcon: String {
        switch model.look.font.priority {
        case .primary:
            return "A"
        case .secondary:
            return "B"
    ...
    var layoutIcon: String {
        model.look.defaultLayout.displayName
    }
    ...
    var surfaceColor: Color {
        let keypath: KeyPath<SurfaceProviderProtocol, Color>
        switch model.look.defaultSurface {
        case .normal:
            keypath = \.normal
        case .active:
            keypath = \.active
        ...
        let color = lookPalette(color: keypath)
    ...
    var activeColor: Color {
        lookPalette(color: \.active)
    }

    func didTapBack() {
        model.isPresentingComponent ?
            model.resetPresentingComponent()
            :
            intentToDismiss()
    }

    func intentToDismiss() {
        model.dismiss()
    }

    func didTapComponents() {}

    var trait: Trait {
        Trait().tweak(.activeSurface)
    }

    var eyeIcon: String {
        model.showControls ? "eye" : "ellipsis"
    }
    ...
    var title: String {
        "\(String(model.look.name.prefix(13))) \(model.componentDetail?.section.displayName ?? "")"
    }

    func tapBack() {
        ATracker.track(.editor(.tapBack))
    }

    func tapLookTitle() {
        ATracker.track(.editor(.tapLookTitle))
    }

    func tapDocs() {
        ATracker.track(.editor(.tapDocs))
        model.onBookTap?()
    }

    func tapPublish() {
        ATracker.track(.editor(.tapPublish))
        model.onShareLookTap?(model.look)
    }

    func tapPreview() {
        ATracker.track(.editor(.tapPreview))
        model.hideControls.toggle()
    }

    var schemeName: String {
        model.isDark ? "Dark Spec" : "Light Spec"
    }

    func tapColorScheme() {
        ATracker.track(.editor(.tapColorScheme))
        model.toggleScheme()
    }

    var body: some View {
        HStack(spacing: 0) {
            Button(action: { self.didTapBack() }) {
                Image(systemName: "chevron.left")
                    .imageScale(.medium)
                    .frame(minWidth: 36, maxWidth: 42)
                    .lux
                    .trait(self.model.trait)
                    .tweak(.activeSurface)
                    .tweak(.bodyLayout)
                    .style(.icon)
                    .feature(.rectangularContentShape)
                    .if(self.model.hideControls) { $0.feature(.invisible) }
                    .view
            }
            Spacer()
            ...
            Text(self.title)
                ...
                .lux
                .trait(self.model.trait)
                .tweak(.activeSurface, .captionLayout)
                .tweak(.bodyLayout)
                ...
                .animation(.none)
                .onTapGesture {
                    ATracker.track(.editor(.tapEditLookMeta))
                    self.model.state.isEditingMeta = true
                }
                .lux
                .if(self.model.hideControls) { $0.feature(.invisible) }
                .view
            ...
            self.model.showControls ?
                Group {
                    Button(action: self.tapDocs) {
                        Image(systemName: "book")
                            ...
                            .tweak(.bodyLayout)
                            .style(.iconLarge)
                            .feature(.rectangularContentShape, .inactiveOpacity)
                            .view
                    }
                }
                : nil

            Button(action: self.tapPreview) {
                Image(systemName: self.eyeIcon)
                    .imageScale(.medium)
                    .lux
                    .trait(self.model.trait)
                    .tweak(.activeSurface)
                    ...
                    .style(.iconLarge)
                    .feature(.rectangularContentShape)
                    .if(self.model.showControls) { $0.feature(.inactiveOpacity) }
                    .view
            }
            .animation(.none)

            Button(action: { self.tapColorScheme() }) {
                Image(systemName: modeIcon)
                    ...
                    .if(self.model.hideControls) { $0.feature(.inactiveOpacity) }
                    .view
            }
        }
    }
...
struct NavigationMenuTuning: View {
    @ObservedObject var model = EditorModel()
    var light = false
...
struct StylingCompact: View {
    ...
    var surfaceIcon: String {
        model.look.defaultSurface.displayName
    }

    var surface: Look.Surface {
        model.look.defaultSurface
    }
    ...
    func tapNextFont() {
        ATracker.track(.editor(.tapFontPrio))
        model.nextFont()
    }

    func tapNextColor() {
        ATracker.track(.editor(.tapColorPrio))
        model.nextColor()
    }

    var body: some View {
        HStack {
            Button(action: self.tapNextFont) {
                Text(self.fontIcon)
                    .lux
                    .trait(self.trait)
                    .tweak(.bodyLayout)
            ...
            Button(action: self.tapNextColor) {
                Image(systemName: self.colorIcon)
                    .imageScale(.large)
                    .lux
                    .trait(self.trait)
                    .tweak(.bodyLayout)
