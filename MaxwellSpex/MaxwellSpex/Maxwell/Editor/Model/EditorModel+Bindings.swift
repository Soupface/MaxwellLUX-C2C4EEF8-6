...
//
//  Licencing info 
//  FileHash: 791D43E1-DD8D-4EE0-AB7E-942C91947AA6
...
extension EditorModel {
    var editingValueBinding: Binding<EffectValue> {
        effectValue(keyPath: editingMode.effectValueKeypath)
    }

    func effectValue(keyPath: WritableKeyPath<Look.Specification, EffectValue>) -> Binding<EffectValue> {
        Binding<EffectValue>(
            get: {
                let effectValue = self.look.spec[keyPath: keyPath]
                return effectValue

            },
            set: { (newValue: EffectValue) in
                self.look.spec[keyPath: keyPath] = newValue
            }
        )
    }
}
...
extension EditorModel {
    var editingColorBinding: Binding<Color> {
        colorBinding(keyPath: editingMode.colorValueKeypath)
    }

    func colorBinding(keyPath: WritableKeyPath<Look.PaletteScheme, Color>, _ scheme: Look.Scheme? = nil) -> Binding<Color> {
        Binding<Color>(
            get: {
                switch scheme {
                case .light:
                    return self.look.specLight.color[keyPath: keyPath]
                case .dark:
                    return self.look.specDark.color[keyPath: keyPath]
                default:
                    return self.look.spec.color[keyPath: keyPath]
                }

            },
            set: { (newValue: Color) in

                switch scheme {
                case .light:
                    self.look.specLight.color[keyPath: keyPath] = newValue
                case .dark:
                    self.look.specDark.color[keyPath: keyPath] = newValue
                default:
                    self.look.spec.color[keyPath: keyPath] = newValue
                }
            }
        )
    }
...
extension EditorModel {
    var editingFontBinding: Binding<UIFont> {
        fontBinding(keyPath: editingMode.fontValueKeypath)
    }

    func fontBinding(keyPath: WritableKeyPath<Look.Fonts, UIFont>, _ scheme: Look.Scheme? = nil) -> Binding<UIFont> {
        Binding<UIFont>(
            get: {
                switch scheme {
                case .light:
                    return self.look.specLight.font[keyPath: keyPath]
                case .dark:
                    return self.look.specDark.font[keyPath: keyPath]
                default:
                    return self.look.spec.font[keyPath: keyPath]
                }

            },
            set: { (newValue: UIFont) in
                switch scheme {
                case .light:
                    self.look.specLight.font[keyPath: keyPath] = newValue
                case .dark:
                    self.look.specDark.font[keyPath: keyPath] = newValue
                default:
                    self.look.spec.font[keyPath: keyPath] = newValue
                }
            }
        )
    }
...
extension EditorModel {
    var galleryOffsetBinding: Binding<CGPoint> {
        Binding<CGPoint>(
            get: {
                .zero
            },
...
extension EditorModel {
    var editingName: Binding<String> {
        Binding<String>(get: { [weak self] in
            self?.look.name ?? ""
        }) { [weak self] in
            self?.look.name = $0
            self?.state.setNeedsUpdate()
        }
    }

    var editingValueStringBinding: Binding<String> {
        let slider = editingValueBinding
        return Binding<String>(
            get: { slider.wrappedValue.displayValue },
            set: { (newValue: String) in
                slider.currentNormal.wrappedValue = CGFloat(Double(newValue) ?? 0)
            }
        )
    }

    var editingValueFlowStringBinding: Binding<String> {
        let slider = editingValueBinding
        return Binding<String>(
            get: { slider.wrappedValue.displayFlow },
            set: { (newValue: String) in
                slider.flowNormalDisplay.wrappedValue = CGFloat(Double(newValue) ?? 0)
            }
        )
    }

    var editingValueMixStringBinding: Binding<String> {
        let slider = editingValueBinding
        return Binding<String>(
            get: { slider.wrappedValue.displayMix },
            set: { (newValue: String) in
                slider.mixNormal.wrappedValue = CGFloat(Double(newValue) ?? 0)
            }
        )
    }
}
...
extension EditorModel {
    var editingColorStringBinding: Binding<String> {
        let colorBinding = self.colorBinding(keyPath: editingMode.colorValueKeypath)

        return Binding<String>(
            get: {
                String((self.state.textInputBuffer ?? colorBinding.wrappedValue.uiColor().hexValue(alwaysIncludeAlpha: false)).prefix(6))

            },
            set: { (newValue: String) in
                let value = "#\(newValue.replacingOccurrences(of: "#", with: ""))"

                self.state.textInputBuffer = String(newValue.prefix(6))
                colorBinding.wrappedValue = UIColor(hex: value)?.uiColor ?? colorBinding.wrappedValue
            }
        )
    }

    func colorStringBinding(keyPath: WritableKeyPath<Look.PaletteScheme, Color>, _ scheme: Look.Scheme? = nil) -> Binding<Color> {
        Binding<Color>(
            get: {
                switch scheme {
                case .light:
...
extension EditorModel {
    func copyFromCurrentBinding() {
        if isPaletteEffect {
            Clipboard.copy(color: editingColorBinding.wrappedValue)
            return
        }
        if isEffectValue {
            Clipboard.copy(string: editingValueBinding.wrappedValue.asJSONString())
            return
        }
    }

    func pasteToCurrentBinding() {
        if isPaletteEffect, let color = Clipboard.pastedColor() {
            editingColorBinding.wrappedValue = color
            return
        }

        if isEffectValue,
            let jsonString = Clipboard.pastedString(),
            let value: EffectValue = Codec.object(fromJSON: jsonString) {
            editingValueBinding.wrappedValue = value
            return
        }
    }
}
