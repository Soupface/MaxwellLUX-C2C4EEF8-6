...
//
//  Licencing info 
//  FileHash: 526AB7EC-7624-4611-A259-4FCE68CB4141
...
extension EditorModel {
    struct Session: Codable {
        ...
        init(tuningLook: Look? = nil, look: Look = Look(), original: Look = Look()) {
            self.look = look
            self.original = original
            self.tuningLook = tuningLook
        }
    }
}
...
extension EditorModel {
    func saveCurrentSession() {
        Self.storageDB.store(item: state.session)
    }
}

extension EditorModel: StorableItemOptional {
    static var storageTable: Storage.Table {
        Storage.Table(name: "editor")
    }

    static var storageInitializer: () -> EditorModel.Session? {
        { nil }
    }

    static let storageDB = Storage.Database<EditorModel.Session?>(table: storageTable) { storageInitializer() }
}
