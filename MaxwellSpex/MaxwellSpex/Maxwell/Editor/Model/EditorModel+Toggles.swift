...
//
//  Licencing info 
//  FileHash: 00ED8E27-989B-40BE-85C9-FAD3D4666A65
...
extension EditorModel {
    var sections: [EditorMode.Gallery] {
        [
            .spex,
            .card,
            .controls,
...
extension EditorModel {
    ...
    func nextLayout() {
        layoutIndex += 1
        let total = layouts.count
        let index = layoutIndex % total
        look.defaultLayout = layouts[index]
    }

    func nextSurface() {
        surfaceIndex += 1
        let total = surfaces.count
        let index = surfaceIndex % total
        look.defaultSurface = surfaces[index]
    }
}

extension EditorModel {
    var priorities: [Look.Priority] {
        [
            .primary,
            .secondary,
            .tertiary,
    ...
    func nextColor() {
        colorIndex += 1
        let total = priorities.count
        let index = colorIndex % total
        look.color.preferred = priorities[index]
    }

    func nextFont() {
        fontIndex += 1
        let total = priorities.count
        let index = fontIndex % total
        look.font.preferred = priorities[index]
    }
}
