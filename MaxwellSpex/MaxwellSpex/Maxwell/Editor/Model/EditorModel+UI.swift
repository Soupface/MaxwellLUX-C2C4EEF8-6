...
//
//  Licencing info 
//  FileHash: 728010DF-D00F-4998-8846-B002DAF3FAF7
...
extension EditorModel {
    var trait: Trait {
        get {
            Trait(
                layout: look.defaultLayout,
                surface: look.defaultSurface,
                fontPriority: look.font.priority,
                colorPriority: look.color.priority,
                fontMix: .byPriority
            )
        }
        set {
            look.defaultLayout = newValue.layout
            look.defaultSurface = newValue.surface
            look.font.preferred = newValue.fontPriority
            look.color.preferred = newValue.colorPriority
        }
    }
}

extension EditorModel {
    var traitNormal: Trait {
        get {
            trait.tweak(.normalSurface)
        }
        set {
            trait = newValue
        }
...
extension EditorModel {
    var barsPadding: CGFloat {
        max(look.padding.base, 28)
    }

    var componentsMenuHeight: CGFloat {
        bottomMenuHeight
    }

    var bottomMenuHeight: CGFloat {
        barsPadding * 1.5
    }

    var componentsMenuOffset: CGFloat {
        isStyling ? bottomMenuHeight : bottomMenuHeight + toolingMenuHeight
    }

    var toolingMenuHeight: CGFloat {
        barsPadding * 3.0
    }
...
extension EditorModel {
    var specName: String {
        "\(look.defaultLayout.displayName) \(look.defaultSurface.displayName) \(look.color.specName) \(look.font.specName)"
    }
}
...
extension EditorModel {
    ...
    struct Icons {
        ...
        func mixDislayView(_ value: CGFloat, line: CGFloat = 2) -> some View {
            display(icon: mix, value: value, line: line)
        }

        func valueDislayView(_ value: CGFloat, line: CGFloat = 2) -> some View {
            display(icon: self.value, value: value, line: line)
        }

        func flowDislayView(_ value: CGFloat, line: CGFloat = 2) -> some View {
            display(icon: flow, value: value, line: line)
        }

        func display(icon: String, value: CGFloat, line: CGFloat = 2) -> some View {
...
extension EditorModel {
    struct Mocking {
        var textBodySamples: [String] {
            [
                "Thoroughly conscious ignorance is the prelude to every real advance in science.",
                "James Clerk Maxwell lived from 13 June 1831 to 5 November 1879. He is widely regarded as one of greatest scientists of any era. His work on the theory of electromagnetism makes him the father of modern physics and he also made fundamental contributions to mathematics, astronomy and engineering.",
                "During an 1861 Royal Institution lecture on colour theory, Maxwell presented the world's first demonstration of colour photography by this principle of three-colour analysis and synthesis. Thomas Sutton, inventor of the single-lens reflex camera, did the actual picture in 1861.",
                "The special theory of relativity owes its origins to Maxwell's equations of the electromagnetic field. -- Albert Einstein",
            ]
        }

        var randomTextBody: String {
            textBodySamples.randomElement() ?? "Body"
        }
    }

    static let mocking = Mocking()
    var mocking: Mocking {
        Self.mocking
    }
}
...
extension EditorModel {
    var editingColorSupportsAlpha: Bool {
//        if editingColorBinding.wrappedValue.uiColor().rgba.alpha < 0.999 {
//            return true
//        }
        switch editingMode {
        case .label, .fillColor, .shadowColor, .tintColor:
            return true
        default:
            return false
        }
...
extension EditorModel {
    var layoutSpacing: CGFloat {
        min(look.padding.base * 2.0, 36)
    }
}

extension Look {
    var hasCustomManifest: Bool {
        meta.manifest?.count ?? 0 > 0
    }

    var hasCustomCover: Bool {
        meta.coverURL?.count ?? 0 > 3
    }

    var defaultCoverURL: URL {
        URL.optional(from: FireConfig.env.editorDefaultLookCoverURL)!
    }

    var defaultLinkURL: URL {
        URL.optional(from: "http://maxwell.design")!
    }

    var displayAuthor: String {
        meta.authorName ?? "James C. Maxwell"
    }

    var displayLemma: String {
        meta.lemma ?? "A NEW LUX"
    }
    ...
    var displayCoverURL: URL {
        guard let cover = meta.coverURL else { return defaultCoverURL }
        return URL.optional(from: cover) ?? defaultCoverURL
    }

    var displayLinkURL: URL {
        guard let linkURL = meta.authorURL else { return defaultLinkURL }
        return URL.optional(from: linkURL) ?? defaultLinkURL
    }

    var displayLinkName: String {
        URLComponents(url: displayLinkURL, resolvingAgainstBaseURL: false)?.host ?? "maxwell.design"
    }
}
