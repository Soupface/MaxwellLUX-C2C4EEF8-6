...
//
//  Licencing info 
//  FileHash: BD496FCF-88E5-42D5-B289-21669354C166
...
extension EditorModel {
    func syncFontPicker() {
        switch editingMode {
        case let .font(prio: prio):
            look.font.preferred = prio
        default:
            break
        }
    }

    func syncControlSubsurface() {
        preferredControlPreviewSurface = nil

        switch editingMode {
        case let .color(_, prio: prio):
            editingMode = .color(look.scheme, prio: prio)
            look.color.preferred = prio
            look.defaultSurface = .active
            preferredControlPreviewSurface = .active

        case let .background(_, prio: prio):
            editingMode = .background(look.scheme, prio: prio)
            switch prio {
            case .primary:
                look.defaultSurface = .canvas
                preferredControlPreviewSurface = .canvas
            case .secondary:
                look.defaultSurface = .normal
                preferredControlPreviewSurface = .normal
            case .tertiary:
                look.defaultSurface = .accent
                preferredControlPreviewSurface = .accent
            default:
                break
            }

        case let .label(_, prio: prio):
            editingMode = .label(look.scheme, prio: prio)

        case .fillColor, .fill, .fillLayout:
            look.defaultSurface = .active
            preferredControlPreviewSurface = .active

//        case .elevation, .tint, .tintColor, .tintLayout:
//            look.preferredSurface == .canvas ? look.preferredSurface = .enabled : nil
//            controlPreviewSurface = .enabled
        default:
            break
        }

        colorPicker?.selectedColor = editingColorBinding.wrappedValue.uiColor()
    }
}
