...
//
//  Licencing info 
//  FileHash: B941C032-6919-45EB-8A34-D41C049B0F8C
...
struct FontTools: View {
    ...
    func fontName(prio: Look.Priority) -> String {
        switch prio {
        case .primary:
            return model.look.font.primary.familyName
        case .secondary:
            return model.look.font.secondary.familyName
        case .tertiary:
            return model.look.font.tertiary.familyName
        default:
            return model.look.font.primary.familyName
        }
    }
    ...
    var body: some View {
        Column {
            Row(spacing: self.model.layoutSpacing * 2.0) {
                ...
                Row {
                    HStack {
                        Button(action: { self.model.setEditingMode(.font(prio: .primary), previewOnly: self.preview) }) {
                            Text("Aa")
                                .scaledToFill()
                                .lux
                                .trait(self.model.traitNormal)
                                .tweak(.primaryFont)
                                .style(.iconLarge, .text, .buttonLarge)
                                .view
                        }

                        Button(action: { self.model.setEditingMode(.font(prio: .secondary), previewOnly: self.preview) }) {
                            Text("Bb")
                                .scaledToFill()
                                .lux
                                .trait(self.model.traitNormal)
                                .tweak(.secondaryFont)
                                .style(.text, .iconLarge, .buttonLarge)
                                .view
                        }

                        Button(action: { self.model.setEditingMode(.font(prio: .tertiary), previewOnly: self.preview) }) {
                            Text("Cc")
                                .scaledToFill()
                                .lux
                                .trait(self.model.traitNormal)
