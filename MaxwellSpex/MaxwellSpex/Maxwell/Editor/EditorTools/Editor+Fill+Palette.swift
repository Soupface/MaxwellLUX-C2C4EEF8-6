...
//
//  Licencing info 
//  FileHash: 0C35FEBC-068E-43BA-AED2-259A02ED4DA4
        ...
        Column {
            ...
            Row {
                Row {
                    CircleColorButton(
                        model: self.model,
                        mode: .fillColor(.light, prio: .primary)
                    )

                    CircleColorButton(
                        model: self.model,
                        mode: .fillColor(.dark, prio: .primary)
                    )
                }
            }
        }
...
struct FillPaletteButton: View {
    @ObservedObject var model: EditorModel
    var scheme: Look.Scheme
    var prio: Look.Priority
    var color: Color

    func icon(_: EditorMode.Editor) -> String {
        switch prio {
        case .primary:
            return "light.max"
        case .secondary:
    ...
    var mode: EditorMode.Editor {
        .fillColor(scheme, prio: prio)
    }

    var body: some View {
        self.model.look.scheme == scheme ?

            Button(action: { self.model.setEditingMode(self.mode, previewOnly: false) }) {
                Image(systemName: self.icon(self.mode))
                    .lux
                    .feature(.foregroundColor)
                    .unless(self.model.editingMode == self.mode) { $0.feature(.disabledOpacity) }
                    .tweak(.titleLayout)
                    .style(.paragraph)
                    .view
