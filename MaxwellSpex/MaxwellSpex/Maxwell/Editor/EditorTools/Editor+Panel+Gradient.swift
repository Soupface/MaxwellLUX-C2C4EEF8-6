...
//
//  Licencing info 
//  FileHash: DDDBAB97-6F70-4A17-994B-53BB7EE07631
...
struct TextureConfigLayer: View {
    @ObservedObject var model: EditorModel
    @State var knob: Knob = .start
    ...
    func setKnob(_ knob: Knob) {
        self.knob = knob
    }

    var currentKnobPointUnit: Binding<UnitPoint> {
        Binding<UnitPoint>(
            get: {
                switch self.knob {
                case .start:
                    return self.model.look.texture.start
                case .finish:
                    return self.model.look.texture.end
                }
            }, set: {
                switch self.knob {
                case .start:
                    self.model.look.texture.start = $0
                case .finish:
                    self.model.look.texture.end = $0
                }
            }
        )
    }

    func unitPointKnob(_ unitPoint: UnitPoint) -> some View {
        Button(action: { self.currentKnobPointUnit.wrappedValue = unitPoint }) {
            Image(systemName: self.currentKnobPointUnit.wrappedValue == unitPoint ? "lightbulb" : "dot.square")
                .imageScale(.large)
                .lux
                .unless(self.currentKnobPointUnit.wrappedValue == unitPoint) { $0.feature(.disabledOpacity) }
                .view
                .scaleEffect(scale)
        }
    }
    ...
    var body: some View {
        Column {
            Group {
                Column {
                    Row {
                        Text("\(self.knob.rawValue.capitalized) at \(self.currentKnobPointUnit.wrappedValue.name)")
                            .lux
                            .tweak(.titleLayout, .activeSurface)
                            .style(.icon, .paragraph)
                            .view
                    }
                    Row {
                        Group {
                            self.unitPointKnob(.topLeading)
                            self.unitPointKnob(.top)
                            self.unitPointKnob(.topTrailing)
                        }
                        .lux
                        .feature(.flexibleFrame)
                        .view
                    }
                    Row {
                        Group {
                            self.unitPointKnob(.leading)
                            self.unitPointKnob(.center)
                            self.unitPointKnob(.trailing)
                        }
                        .lux
                        .feature(.flexibleFrame)
                        .view
                    ...
                    Row {
                        Group {
                            self.unitPointKnob(.bottomLeading)
                            self.unitPointKnob(.bottom)
                            self.unitPointKnob(.bottomTrailing)
                        }
                        .lux
                        .feature(.flexibleFrame)
                        .view
                ...
                Row {
                    Group {
                        Button(action: { self.setKnob(.start) }) {
                            Column {
                                Row {
                                    CircleColorButton(
                                        model: self.model,
                                        mode: .tintColor(.light, prio: .primary),
                                        preferredIcon: "lightbulb"
                                    )

                                    CircleColorButton(
                                        model: self.model,
                                        mode: .tintColor(.dark, prio: .primary),
                                        preferredIcon: "lightbulb"
                                    )
                                ...
                                Row {
                                    Text("Start")
                                        .lux
                                        .style(self.model.styles.captionStyle)
                                        .view
                                }
                            }
                            .lux
                            .feature(.flexibleWidth, .padding)
                            .unless(self.knob == .start) { $0.feature(.inactiveOpacity) }
                            .view
                        }

                        Button(action: { self.setKnob(.finish) }) {
                            Column {
                                Row {
                                    CircleColorButton(
                                        model: self.model,
                                        mode: .tintColor(.light, prio: .secondary),
                                        preferredIcon: "lightbulb"
                                    )
                                    CircleColorButton(
                                        model: self.model,
                                        mode: .tintColor(.dark, prio: .secondary),
                                        preferredIcon: "lightbulb"
                                    )
                                }.allowsHitTesting(false)
                                Row {
                            ...
                            .lux
                            .feature(.flexibleWidth, .padding)
                            .unless(self.knob == .finish) { $0.feature(.inactiveOpacity) }
                            .view
                        }
                    }
                }
...
//                Row {
//                    Button(action: { self.model.isConfiguringTexture = false }) {
//                        Text("OK")
//                            .bold()
//                            .lux
//                            .tweak(.activeSurface)
...
struct TextureConfigLayer_Previews: PreviewProvider {
    static var previews: some View {
        TextureConfigLayer(model: EditorModel())
    }
}
