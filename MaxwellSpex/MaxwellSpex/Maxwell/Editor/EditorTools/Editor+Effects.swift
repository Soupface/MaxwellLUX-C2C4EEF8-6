...
//
//  Licencing info 
//  FileHash: 55B52980-6C65-47FF-80E6-1180D312407A
...
struct EffectsMenu: View {
    @ObservedObject var model: EditorModel

    var body: some View {
        Row(spacing: self.model.layoutSpacing * 2.0) {
            Spacer()
                .frame(minWidth: self.model.layoutSpacing)
            ...
            Row {
                Image(systemName: "square.stack.3d.up.fill")
                    ...
                    .view
                    .padding(4)
                    .background(Circle().stroke(self.lux.spec.surface.active))
                    .lux
                    .feature(.shadow)
                    .view
            ...
            Row {
                TexturePalette(model: self.model, editor: true)
                ...
                TexturePropertyView(model: self.model)
            }

            Row {
                ShadowPalette(model: self.model, editor: true)
                EffectValueDisplay(
                    model: self.model,
                    mode: .shadow,
                    hideTitle: true
...
struct TexturePropertyView: View {
    @ObservedObject var model: EditorModel

    var body: some View {
        Column {
            HStack {
                Button(action: { self.model.isConfiguringTexture = true }) {
        ...
        Column {
            ...
            Row {
                Row {
                    CircleColorButton(
                        model: self.model,
                        mode: .shadowColor(.light, prio: .primary)
                    )

                    CircleColorButton(
                        model: self.model,
                        mode: .shadowColor(.dark, prio: .primary)
                    )
                }
            }
        }
