...
//
//  Licencing info 
//  FileHash: 9BAD7F19-75AA-4426-A042-D5A75225A67C
...
struct EditingTools: View {
    ...
    var offset: CGFloat {
        -model.toolingMenuHeight + model.componentsMenuHeight
    }
    ...
    var body: some View {
        Column {
            ...
            Spacer()
                .lux
                .if(self.model.isTuning) { $0.tweak(.canvasSurface) }
                .feature(.separatorFrame)
                .view

            EditingMenu(model: self.model)
                .lux
        ...
        ZStack {
            Column {
                ScrollView(.horizontal, showsIndicators: false) {
                    PaletteMenu(model: self.model)
                        .frame(minHeight: self.model.toolingMenuHeight)
                }
                .frame(height: self.model.toolingMenuHeight)
            }
            .lux
            .unless(self.model.editingModeGroup == .color) { $0.feature(.invisible) }
            .view

            Column {
                ScrollView(.horizontal, showsIndicators: false) {
                    EffectsMenu(model: self.model)
                        .frame(minHeight: self.model.toolingMenuHeight)
                }
                .frame(height: self.model.toolingMenuHeight)
            }
            .lux
            .unless(self.model.editingModeGroup == .fx) { $0.feature(.invisible) }
            .view

            Column {
                ScrollView(.horizontal, showsIndicators: false) {
                    FontTools(model: self.model)
                        .frame(minHeight: self.model.toolingMenuHeight)
                }
                .frame(height: self.model.toolingMenuHeight)
            }
            .lux
            .unless(self.model.editingModeGroup == .font) { $0.feature(.invisible) }
            .view
        }
        .frame(idealHeight: self.model.toolingMenuHeight)
    }
}
