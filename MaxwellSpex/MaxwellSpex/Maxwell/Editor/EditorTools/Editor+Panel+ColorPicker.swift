...
//
//  Licencing info 
//  FileHash: E1365B0C-62B6-462D-A184-1082098E0DF6
...
struct ColorPickerPanel: View {
    @ObservedObject var look = Look.current
    @ObservedObject var model: EditorModel
    @ObservedObject var preferences = Preferences.shared

    var trait: Trait {
        model.trait.tweak(.bodyLayout, .canvasSurface)
    }

    var titleBackgroundColor: Color {
        lookPalette(color: \.normal).opacity(0.75)
    }

    var backgroundColor: Color {
        lookPalette(color: \.normal).opacity(0.25)
    }

    var preferenceKey: String {
        "ColorPicker:editing:mode"
    }

    var mode: DynamicColorPickerViewController.Mode {
        if let pref = self.model.preferences[preferenceKey] as? String {
            return DynamicColorPickerViewController.Mode(rawValue: pref) ?? .hue
        }
        return .hue
    }

    func setPreferred(mode: DynamicColorPickerViewController.Mode) {
        model.preferences[preferenceKey] = mode.rawValue
    }

    func dismiss() {
        preferences.state.recentColors.append(model.editingColorBinding.wrappedValue)
    ...
    var body: some View {
        Column {
            Column {
                Row {
                    self.model.editingColorSupportsAlpha ?
                        Button(action: { self.setPreferred(mode: .alpha) }) {
                            Text("ALPHA")
                                .bold()
                                .lux
                                .trait(self.trait)
                                .feature(.paragraph)
                                .unless(self.mode == .alpha) { $0.feature(.inactiveOpacity) }
                                .tweak(.edgesVertical)
                                .feature(.padding)
                                .view
                        }
                    ...
                    Button(action: { self.setPreferred(mode: .rgb) }) {
                        Text("RGB")
                            .bold()
                            .lux
                            .trait(self.trait)
                            .feature(.paragraph)
                            .unless(self.mode == .rgb) { $0.feature(.inactiveOpacity) }
                            .tweak(.paddingHalf, .edgesVertical)
                            .feature(.padding)
                            .view
                    }

                    Button(action: { self.setPreferred(mode: .hue) }) {
                        Text("HUE")
                            .bold()
                            .lux
                            .trait(self.trait)
                            .feature(.paragraph)
                            .unless(self.mode == .hue) { $0.feature(.inactiveOpacity) }
                            .tweak(.paddingHalf, .edgesVertical)
                            .feature(.padding)
                            .view
                    }

                    Button(action: { self.setPreferred(mode: .hsb) }) {
                        Text("HSB")
                            .bold()
                            .lux
                            .trait(self.trait)
                            .feature(.paragraph)
                            .unless(self.mode == .hsb) { $0.feature(.inactiveOpacity) }
                            .tweak(.paddingHalf, .edgesVertical)
                            .feature(.padding)
                            .view
                    }

                    Button(action: { self.setPreferred(mode: .featured) }) {
                        Image(systemName: "square.grid.4x3.fill")
                            .lux
                            .trait(self.trait)
                            .feature(.paragraph, .shadow)
                            .unless(self.mode == .rgb) { $0.feature(.inactiveOpacity) }
                            .tweak(.paddingHalf, .edgesVertical)
                            .feature(.padding)
                            .view
            ...
            .view
            .background(
                self.lux.spec.surface.canvas.uiColor().withAlphaComponent(0.5).uiColor
            )

            Group {
                self.mode != .featured ?
                    ColorPickerControllerUI(model: self.model, mode: self.mode, color: self.model.editingColorBinding.wrappedValue)
                    .lux
                    .view
                    : FeaturedColorPalette(model: self.model)
                    .id(self.model.state.stateHash)
                    .lux
                    .view
            }
            .id("com.maxwell.colorPicker.\(self.mode.rawValue)")
            .id(self.model.editingMode.identifier)
            .frame(minHeight: 0.0, maxHeight: self.model.layoutSpacing * 6, alignment: .center)

            Row {
                Button(action: { self.model.state.textInputBinding = self.model.editingColorStringBinding }) {
                    Text("#\(String(self.model.editingColorStringBinding.wrappedValue.prefix(6)))")
                        .lineLimit(1)
                        .fixedSize()
                        .frame(minWidth: 66)
                        .lux
                ...
                RowPanelConfirm(trait: self.trait, action: self.dismiss)
                    .lux
                    .feature(.flexibleWidth)
                    .view
            }
        ...
        .lux
        .trait(trait.preferred(color: self.backgroundColor))
        .style(.panel)
        .view
    }
}

struct FeaturedColorPalette: View {
    struct Header {
        var icon: String
        var color: Color
        var iconColor: Color
    ...
    func apply(color: Color) {
        preferences.state.recentColors.append(colorBinding.wrappedValue)
        preferences.state.recentColors.append(color)
        colorBinding.wrappedValue = color
    }

    var lightSpec: SpecificationProtocol {
        model.look.specLight
    }

    var lightColors: [Color] {
        [
            lightSpec.surface.canvas,
            lightSpec.surface.normal,
            lightSpec.surface.accent,
            lightSpec.color.primary,
            lightSpec.color.secondary,
            lightSpec.color.tertiary,
            lightSpec.surface.foregroundLight,
            lightSpec.surface.foregroundDark,
        ]
    }

    var darkSpec: SpecificationProtocol {
        model.look.specDark
    }

    var darkColors: [Color] {
        [
            darkSpec.surface.canvas,
            darkSpec.surface.normal,
            darkSpec.surface.accent,
            darkSpec.color.primary,
            darkSpec.color.secondary,
            darkSpec.color.tertiary,
            darkSpec.surface.foregroundLight,
            darkSpec.surface.foregroundDark,
        ]
    }

    var recentColors: [Color] {
        preferences.state.recentColors.reversed()
    }
    ...
    var headers: [Header] {
        [
            Header(icon: "sun.max", color: lightSpec.surface.active, iconColor: lightSpec.surface.active.contrast),
            Header(icon: "moon", color: darkSpec.surface.active, iconColor: darkSpec.surface.active.contrast),
            Header(icon: "clock", color: .clear, iconColor: model.look.surface.active),
        ]
    }
    ...
    var body: some View {
        Column {
            Group {
                ForEach(Array(self.colors.enumerated()), id: \.element) { idx, colors in
                    ScrollView(.horizontal, showsIndicators: false) {
                        Row {
                            ColorCircleButton(color: self.headers[idx].color, icon: self.headers[idx].icon, iconColor: self.headers[idx].iconColor) {}

                            ForEach(colors, id: \.self) { color in
                                ColorCircleButton(color: color, icon: "", iconColor: Color.white)
                                { self.apply(color: color) }
                            }
                            .lux
                            .style(.iconLarge)
                            .view
            ...
            .lux
            .tweak(.edgesHorizontal)
            .feature(.flexibleWidth, .controlLayout)
            .view
        }
    }
}
