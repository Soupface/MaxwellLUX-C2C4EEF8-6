...
//
//  Licencing info 
//  FileHash: ACF3BA84-B4E6-4B20-83D6-284782C438F6
...
struct TemplatesContainerView: View {
    @ObservedObject var model: AppModel
    @ObservedObject var content = FireConfig.shared
    ...
    func refresh() {
        FireConfig.refresh()
        BannerModel.shared.push(message: "Looking for new Lux...")
    }
    ...
    var body: some View {
        ZStack {
            Column {
                ...
                TemplatesView(model: self.model) { self.onSelect?($0) }
            }
            BannerTopView()
        }
    }
...
struct TemplatesView: View {
    ...
    @ObservedObject var model: AppModel
    @ObservedObject var content = FireConfig.shared
    @State var section: Section = .ux

    var onSelect: (Look) -> Void
    var ux: [Look] {
        let result = Templates.design
        return result.isEmpty ? [Look()] : result
    }

    var manifests: [Look] {
        let result = Templates.manifest
        return result.isEmpty ? [Look()] : result
    }

    var branded: [Look] {
        let result = Templates.branded
        return result.isEmpty ? [Look()] : result
    }
    ...
    func didSelect(look: Look) {
        ATracker.track(.home(.pickTemplate))
        onSelect(look)
    }
    ...
    var body: some View {
        ZStack {
            NavigationView {
                Form {
                    List {
                        ...
                        ForEach(self.looks, id: \.self) { (look: Look) in
                            Column {
                                LookDisplay(
                                    look: look,
                                    active: false,
                                    height: 123,
                                    onEdit: {
                                        self.didSelect(look: look)
                                    },
                                    animate: false
                                )
                                .lux
                                .trait(Trait(spec: look.spec))
                                .feature(.shadow)
                                .view
                                Spacer()
                                    .frame(height: 66)
                            }
                            .lux.style(.layoutBlock).view
                            .background(Color.clear)
                            .listRowInsets(EdgeInsets())
                            .listRowBackground(Color.clear)
                            .id("tempaltes.look.\(look.meta.creationId)")
                        }
            ...
            Column {
                Spacer()
                Row {
                    Button(action: { self.section = .branded }) {
                        Text("Branded")
                            .bold()
                            .lux
                            .tweak(.activeColor)
                            .tweak(.headlineLayout)
                            .style(.icon, .typography)
                            .feature(.flexibleWidth, .shadow)
                            .view
                    }
                    .lux
                    .unless(self.section == .branded) { $0.feature(.inactiveOpacity) }
                    .view

                    Button(action: { self.section = .ux }) {
                        Text("UX")
                            .bold()
                            .lux
                            .tweak(.activeColor)
                    ...
                    .lux
                    .unless(self.section == .ux) { $0.feature(.inactiveOpacity) }
                    .view

                    Button(action: { self.section = .purpose }) {
                        Text("Purpose")
                            .bold()
                            .lux
                            .tweak(.activeColor)
                    ...
                    .lux
                    .unless(self.section == .purpose) { $0.feature(.inactiveOpacity) }
                    .view
                }
                .lux
                .tweak(.activeColor)
