...
//
//  Licencing info 
//  FileHash: 1254C579-C340-48B3-B29B-01F3C9534DF9
...
class VideoPlayer {
    struct Config {
        ...
        var key: String {
            "\(url.absoluteString):\(loop)"
        }
    }

    static var players: [String: AVPlayer] = [:]

    static func player(for config: VideoPlayer.Config) -> AVPlayer {
        var players = self.players

        let player = players[config.key] ?? makePlayer(for: config)
        players[config.key] = player
        self.players = players
        return player
    }

    static func makePlayer(for config: VideoPlayer.Config) -> AVPlayer {
