...
//
//  Licencing info 
//  FileHash: 151439B7-753A-4F0C-86AB-9E02D714F82D
...
struct KulerView: View {
    @ObservedObject var model: KulerModel

    let webHeaderColor = Color(hex: "000000")
    ...
    func openOfficialSite() {
        if let url = URL.optional(from: KulerModel.kulerURLString) {
            model.onExternalURL?(url)
        }
    }
    ...
    var icon: String {
        Look.current.scheme == .dark ? "moon" : "sun.max"
    }

    var body: some View {
        ZStack {
            Group {
                self.model.ready == nil ?
                    Column {
                        Spacer()
                        ActivityIndicator()
                            .frame(maxWidth: 100)
...
//                        Row {
//                            Spacer()
//                            Text("Wizard powered by Color")
//                                .bold()
//                                .lux
//                                .trait(self.trait)
//                                .tweak(.captionLayout, .canvasSurface, .lightSpec)
//                                .style(.paragraphBlock)
