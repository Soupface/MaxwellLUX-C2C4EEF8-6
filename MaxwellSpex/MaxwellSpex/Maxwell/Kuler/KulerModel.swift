...
//
//  Licencing info 
//  FileHash: 746FAB00-1F40-4A8D-85F7-6DAF485BB0D4
...
class KulerModel: WebViewStore, WKNavigationDelegate, UINavigationControllerDelegate {
    @Published var ready: Bool? = nil
    var onConfirm: (([UIColor]) -> Void)?
    var onCancel: (() -> Void)?
    var looping = false

    static let NAME_TOKEN = "__NAME__"
    static let RGB_TOKEN = "__RGB_VALUES__"
    static let BASE_URL = FireConfig.env.pageKulerURL
    static let kulerURLString = "\(BASE_URL)?base=2&rule=Shades&selected=2&name=\(NAME_TOKEN)&mode=rgb&rgbvalues=\(RGB_TOKEN)&swatchOrder=0,1,2,3,4"
//    static let kulerURLString = "https://kuler.adobe.com/create/color-wheel/?base=2&rule=Shades&selected=2&name=\(NAME_TOKEN)&mode=rgb&rgbvalues=\(RGB_TOKEN)&swatchOrder=0,1,2,3,4"

    init() {
        super.init(rootURLString: KulerModel.kulerURLString)
    ...
    override init(rootURLString: String, webView: WKWebView = WKWebView()) {
        super.init(rootURLString: rootURLString, webView: webView)
        webView.navigationDelegate = self
        loadIfNeeded()
    }

    override var rootURL: URL {
        URL.optional(from: Self.dynamicRootURL)!
    }
}

extension KulerModel {
...
extension KulerModel {
    func JSrunCustomizationLoop() {
        guard looping == false else { return }
        looping = true
        let script = JSbuildCustomizationScript()
        let loop = JSmakeScript(interval: 250, script: script)
        JSperformScript(script: loop)
    }

    func JSapplyCustomization() {
        let script = JSbuildCustomizationScript()
        JSperformScript(script: script)
    }

    func JSbuildCustomizationScript() -> String {
        "document.querySelectorAll('[id=smartbanner]').forEach(e => e.parentNode.removeChild(e));"
//        let info: [RemoveInfo] = [
//            RemoveInfo(tag: "header"),
//            RemoveInfo(tag: "footer"),
//            RemoveInfo(tag: "class", value: "savepanel"),
        ////            RemoveInfo(tag: "class", value: "themepanel"),
//        ]
//
//        return info.reduce("") { "\($0);\(JSselectorComposableString(tag: $1.tag, value: $1.value))\(Self.JSRemoveScriptSufix)" }
    }
}

extension KulerModel {
    func applyOptimisticCustomization() {
        JSapplyCustomization()
    }

    func verifyIntegrity(completion: @escaping (Bool) -> Void) {
        var header: Bool?
        var footer: Bool?
        var savepanel: Bool?
        var themepanel: Bool?
        ...
        let resolve = {
            guard
                ...
                let themepanel = themepanel
            else { return }

            completion(header && footer && savepanel && themepanel)
        }

        JSsearchFor(tag: "header") { header = $0; resolve() }
        JSsearchFor(tag: "footer") { footer = $0; resolve() }
        JSsearchFor(tag: "class", value: "themepanel") { savepanel = $0; resolve() }
        JSsearchFor(tag: "class", value: "savepanel") { themepanel = $0; resolve() }
    }
}

extension KulerModel {
    func resolveColors() {
        extractColors { [weak self] colorStrings in
            guard let this = self else { return }
            this.applyJSColors(
                this.parseColors(from: colorStrings ?? [])
            )
        }
    }

    func extractColors(_ completion: @escaping ([String]?) -> Void) {
        let script = "var result = []; document.querySelectorAll('[class=themeBox] > [style*=background]').forEach(e => result.push(window.getComputedStyle(e).backgroundColor)); result;"
        JSperformScript(script: script) { html in
            if let html = html {
                guard let result = html as? [String] else {
                    completion(nil)
                    return
                }
    ...
    func parseColors(from strings: [String]) -> [UIColor] {
        strings.map { parseRGBColor(string: $0) }
    }

    func applyJSColors(_ colors: [UIColor]) {
        DispatchQueue.main.async {
            self.onConfirm?(colors.suffix(5))
        }
    }
}

extension KulerModel {
    func parseRGBColor(string: String) -> UIColor {
        let rgbValuesString = string.replacingOccurrences(of: "rgb(", with: "").replacingOccurrences(of: ")", with: "")
        let rgbValues = rgbValuesString.components(separatedBy: ",")
        ...
        let red: CGFloat = ((Double(rgbValues[0].alphanumeric) ?? 0.0) / 255.0).cgFloat
        let green: CGFloat = ((Double(rgbValues[1].alphanumeric) ?? 0.0) / 255.0).cgFloat
        let blue: CGFloat = ((Double(rgbValues[2].alphanumeric) ?? 0.0) / 255.0).cgFloat
...
extension KulerModel {
    ...
    func syncIntent() {
        ...
        ready = nil
        colorsInSync { $0 ? self.ready = true : retry() }
    }

    func colorsInSync(_ completion: @escaping (Bool) -> Void) {
        extractColors { [weak self] colorStrings in
            guard
                let this = self,
                let colorStrings = colorStrings
            else {
                completion(false)
                return
            }
            let colors = this.parseColors(from: colorStrings)
            guard colors.count >= 5 else {
                completion(false)
                return
            }

            let kuler = Set<String>(colors.map { $0.hexValue() })
            let look = Set<String>(Self.currentLookColorStrings)

            completion(kuler == look)
        }
    }

    static var currentLookColorStrings: [String] {
        currentLookColors.map { $0.hexValue() }
    }

    static var currentLookColors: [UIColor] {
        [
            Look.current.surface.canvas,
            Look.current.surface.normal,
            Look.current.color.primary,
            Look.current.color.secondary,
            Look.current.color.tertiary,
        ].map { $0.uiColor() }
    }
}

extension KulerModel {
    static var dynamicRootName: String {
        Look.current.name.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed) ?? "LUX"
    }

    static func stride(_ value: CGFloat, size: Double = 255) -> Double {
        trunc(value.doubleValue * size) / size
    }

    static var dynamicRootRGBVALUES: String {
        currentLookColors.map { "\("\($0.rgb.red)".prefix(16)),\("\($0.rgb.green)".prefix(16)),\("\($0.rgb.blue)".prefix(16))" }.joined(separator: ",")
    }

    static var dynamicRootURL: String {
        let template = kulerURLString
        let result = template
            .replacingOccurrences(of: NAME_TOKEN, with: dynamicRootName)
            .replacingOccurrences(of: RGB_TOKEN, with: dynamicRootRGBVALUES)
