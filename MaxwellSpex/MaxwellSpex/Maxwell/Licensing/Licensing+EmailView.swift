...
//
//  Licencing info 
//  FileHash: AA8D2D33-629A-489D-B5BD-6F48838AB768
...
extension LicensingView {
    var emailBinding: Binding<String> {
        Binding<String>(
            get: {
                self.licensing.state.email ?? self.licensing.state.license?.entitlements?.email ?? Authentication.user?.email ?? ""
            },
            set: {
                self.licensing.state.email = $0
            }
        )
    }

    func emailStep() -> some View {
        Column {
            HStack {
            ...
            Row {
                TextField("your@email.com", text: self.emailBinding)
                    .lux
            ...
            Row {
                Text("Final download will be sent to this address")
            }
            .lux
            .tweak(.footerLayout)
            .style(.paragraph)
