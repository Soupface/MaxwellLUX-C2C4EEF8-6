...
//
//  Licencing info 
//  FileHash: ED92FAA6-BF5B-4333-8FBE-136FAA3141BC
...
extension LicensingView {
    var companyBinding: Binding<String> {
        Binding<String>(
            get: {
                self.licensing.state.company ?? self.licensing.state.license?.entitlements?.company ?? ""
            },
            set: {
                self.licensing.state.company = $0
            }
        )
    }

    func companyStep() -> some View {
        Column {
            HStack {
            ...
            Row {
                TextField("Apples Inc", text: self.companyBinding)
                    .lux
                    .style(.textfieldLarge, .paragraph)
                    .view
            }
            .lux
            .style(.paragraph)
            ...
            Row {
                Text("License will be valid for this company")
            }
            .lux
            .tweak(.footerLayout)
            .style(.paragraph)
