...
//
//  Licencing info 
//  FileHash: 5A250299-44AB-46E5-8658-9D2E27617952
...
extension LicensingView {
    func licenseStep() -> some View {
        Column {
            ...
            ScrollView {
                Text(self.licensing.state.contract).lux
                    .style(.paragraph)
                    .view
            }

            Row {
                Text("\(self.licensing.state.name ?? "Notice,") by continuing with the licensing process you explicitely express consent and agreement with this terms.")
            }
            .lux
            .tweak(.activeSurface)
            .tweak(.footerLayout)
            .style(.paragraph, .panel)
            .view
        }
    }
}
