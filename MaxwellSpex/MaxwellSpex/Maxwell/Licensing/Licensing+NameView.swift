...
//
//  Licencing info 
//  FileHash: B9BB897A-2AB3-4EC4-A375-0EA927B5AFDF
...
extension LicensingView {
    var nameBinding: Binding<String> {
        Binding<String>(
            get: {
                self.licensing.state.name ?? self.licensing.state.license?.entitlements?.name ?? Authentication.user?.displayName ?? ""
            },
            set: {
                self.licensing.state.name = $0
            }
        )
    }

    func nameStep() -> some View {
        Column {
            HStack {
            ...
            Row {
                TextField("Jhon Apple", text: self.nameBinding)
                    .lux
            ...
            Row {
                Text("License will be crafter under this name")
            }
            .lux
            .tweak(.footerLayout)
            .style(.paragraph)
