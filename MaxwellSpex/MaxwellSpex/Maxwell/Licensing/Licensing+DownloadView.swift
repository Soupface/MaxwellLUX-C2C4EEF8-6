...
//
//  Licencing info 
//  FileHash: 2BF7C523-8CE8-4BF7-9435-8DD9902240CA
...
//
//
//  Licensing+DownloadView.swift
//  MaxwellSpex
//
//  Created by mark maxwell on 5/10/20.
...
extension LicensingView {
    func qrImage() -> some View {
        Image(uiImage: LUXQR.QRLux(from: licenseURLString, color: lux.spec.surface.canvas.contrast.uiColor()))
            .resizable()
    }

    var licenseURLString: String {
        licensing.state.license?.productFinalURL?.absoluteString ?? FireConfig.env.pageNoLicenseURL
    }

    var title: String {
        licensing.state.isLicenseReady ? "Download Ready" : "Preparing Download"
    }

    var nexActionTitle: String {
        licensing.state.hasValidLicense ? licensing.state.isUploading ? "Creating Product" : "Ready" : "Payment"
    }

    var description: String {
        licensing.state.isUploading ? "Please wait until completion" : licensing.state.selectedProduct.descriptionExtended
    }

    var nexActionStatus: String {
        licensing.state.hasValidLicense ? licensing.state.license?.status ?? "" : "Please continue..."
    }

    func downloadStep() -> some View {
        Column {
            HStack {
            ...
            .lux
            .tweak(.titleLayout)
            .style(.iconLarge, .paragraph)
            .view

            Spacer()

            self.licensing.state.isLicenseReady ?
                Column {
                    Text(self.licensing.state.selectedProduct.titleExtended)
                        .bold()
                        .lux
                    ...
                    HStack {
                        Image(self.licensing.state.selectedProduct.displayImage)
                            .resizable()
                            .scaledToFit()
                            .lux
                            .feature(.cornerRadius, .paragraph)
                            .view
                        ...
                        self.qrImage()
                            ...
                            .feature(.padding)
                            .view
                            .background(self.lux.draw.surfaceComposition)
                            .lux
                    ...
                    HStack {
                        ...
                        Text("Save screenshot for your records")
                            .bold()
                            .lux
                            .tweak(.captionLayout)
                            .style(.icon, .paragraph)
                ...
                Column {
                    ...
                    Column {
                        ...
                        Text(self.licensing.state.selectedProduct.titleExtended)
                            ...
                            .view
                            .multilineTextAlignment(.center)
                        Text(self.licensing.state.selectedProduct.descriptionExtended)
                            .bold()
                            .lux
                            .tweak(.captionLayout)
                            .style(.paragraph)
                    ...
                    self.licensing.state.hasValidLicense == false ?
                        Button(action: self.licensing.performPayment) {
                            Text("Complete Purchase")
                                .scaledToFill()
                                .lux
                                .tweak(.activeSurface)
                                .style(.buttonLarge, .layoutBlock, .text)
                                .view
                        }

                        : nil

                    self.licensing.state.readyToUpload ?
                        Button(action: self.licensing.createAndUploadProduct) {
                            Text("Create Product")
                                .scaledToFill()
                                .lux
                                .tweak(.activeSurface)
                    ...
                    self.licensing.state.isUploading ?
                        ActivityIndicator()
                        .frame(maxWidth: 100)
                        .blendMode(.difference)
                        : nil

                    Text(self.nexActionStatus)
                        .lux
                        .style(.icon, .paragraphBlock)
                        .view
                }
                .lux
                .tweak(.canvasSurface)
                .feature(.backgroundSystemBlur, .cornerRadius)
                .view
