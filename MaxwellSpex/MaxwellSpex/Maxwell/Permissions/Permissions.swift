...
//
//  Licencing info 
//  FileHash: 100B0E86-F3BE-47A7-B151-EC248A0210DB
...
class Permissions: NSObject {
    ...
    static func launchSetup() {
        requestSignInPermissions()
    }

    static func requestSignInPermissions() {
    ...
    static var canRequestPermissions: Bool {
        Authentication.hasSignedUser
    }

    static func requestPhotos(completion: ((Bool) -> Void)? = nil) {
        guard canRequestPermissions else {
            completion?(false)
            return
        }
        ...
        PHPhotoLibrary.requestAuthorization { _ in
            ...
            PHPhotoLibrary.shared().register(ImportModel.shared)
            ImportModel.shared.fetchPhotos(suggestion: true)

            completion?(true)
        }
    }

    static func requestPushNotifications(completion: ((Bool) -> Void)? = nil) {
        guard canRequestPermissions else {
            completion?(false)
            return
        }
        ...
        service.getNotificationSettings { settings in
            switch settings.authorizationStatus {
            // This means we have not yet asked for notification permissions
            case .notDetermined:
                service.requestAuthorization(options: [.alert, .sound, .badge], completionHandler: { granted, error in
                    // You might want to remove this, or handle errors differently in production
                    assert(error == nil)
                    DispatchQueue.main.async {
                        granted ? UIApplication.shared.registerForRemoteNotifications() : nil
                    }
...
extension Permissions: UNUserNotificationCenterDelegate {
    @available(iOS 10.0, *)
